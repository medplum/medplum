...
aws s3 cp index.html s3://www.medplum.com/index.html \
  --region us-east-1 \
...
# Copy sitemap.xml
# Set non-cache so clients revalidate
aws s3 cp sitemap.xml s3://www.medplum.com/sitemap.xml \
  --region us-east-1 \
...
for input_file in **/*.html; do
  ...
  aws s3 cp "$input_file" "s3://www.medplum.com/$output_file" \
    --region us-east-1 \
    --content-type "text/html; charset=utf-8" \
    --cache-control "no-cache"
done
