...
# Construct full version, including the git shorthash
...
node -e "require('fs').copyFileSync(process.execPath, 'dist/medplum-agent-$MEDPLUM_FULL_VERSION-win64.exe')" 
...
if [[ -z "$SIGNTOOL_PATH" ]]; then
  ...
  signtool remove -s dist/medplum-agent-$MEDPLUM_FULL_VERSION-win64.exe
else
  echo "Using signtool located at $SIGNTOOL_PATH"
  # Otherwise use the defined path from the env var
  "$SIGNTOOL_PATH" remove -s dist/medplum-agent-$MEDPLUM_FULL_VERSION-win64.exe
fi
...
npx postject dist/medplum-agent-$MEDPLUM_FULL_VERSION-win64.exe NODE_SEA_BLOB sea-prep.blob --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2 
