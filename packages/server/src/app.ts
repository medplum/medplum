...
import { closeDatabase, initDatabase, queueAllPendingPostDeployMigrations } from './database';
...
export function initAppServices(config: MedplumServerConfig): Promise<void> {
  return requestContextStore.run(AuthenticatedRequestContext.system(), async () => {
    ...
    if (!config.disableAutoRunPostDeployMigrations) {
      await queueAllPendingPostDeployMigrations();
    }
  });
}
