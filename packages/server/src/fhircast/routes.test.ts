...
describe('FHIRcast routes', () => {
  ...
  beforeAll(async () => {
    ...
    expect(res1.shardId).toStrictEqual(res2.shardId);

    accessToken = res1.accessToken;
    project = res1.project;
    shardId = res1.shardId;
  ...
  test('Get context after *-close event', async () => {
    ...
    await setTopicCurrentContext(shardId, project.id, topic, {
      'context.type': 'DiagnosticReport',
  ...
  test('Check for `context.versionId` on `DiagnosticReport-open`', async () => {
    ...
    const latestContextStr = (await getCacheRedis(shardId).get(
