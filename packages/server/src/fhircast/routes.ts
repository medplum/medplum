...
protectedCommonRoutes.post(
  ...
  asyncWrap(async (req: Request, res: Response) => {
    ...
    const topicParam = req.params.topic as string;
    getLogger().debug('FHIRcast event received', {
      topic: topicParam,
      eventType: event['hub.event'],
      payload: req.body,
    });
    ...
    if (event['hub.event'].endsWith('-open')) {
      ...
      await getRedis().set(`medplum:fhircast:topic:${topicParam}:latest`, stringifiedBody);
      ...
      await getRedis().del(`medplum:fhircast:topic:${topicParam}:latest`);
    ...
    await getRedis().publish(topicParam, stringifiedBody);
    getLogger().debug('FHIRcast event published to topic', {
      topic: topicParam,
      eventType: event['hub.event'],
    });
    res.status(201).json({ success: true, event: req.body });
  })
);
