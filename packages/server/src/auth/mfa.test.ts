...
describe('MFA', () => {
  ...
  /**
   * Flags the user as requiring MFA enrollment so that the next password login
   * returns `mfaEnrollRequired` and the `/login-enroll` flow becomes active.
   * Re-reads the user first so any `mfaSecret` generated by `/status` is kept.
   * @param user - The user to update.
   * @param options - Options.
   * @param options.clearSecret - When true, removes the user's `mfaSecret`.
   */
  async function setMfaRequired(user: WithId<User>, options?: { clearSecret?: boolean }): Promise<void> {
    const systemRepo = getGlobalSystemRepo();
    await withTestContext(async () => {
      ...
      await systemRepo.updateResource<User>({
        ...current,
        mfaRequired: true,
        ...(options?.clearSecret ? { mfaSecret: undefined } : {}),
      });
    });
  }
  ...
  test('enroll requires a token for TOTP', async () => {
    ...
    const { accessToken } = await withTestContext(() =>
      registerNew({
        firstName: 'Enroll',
        lastName: 'NoToken',
        projectName: `Enroll No Token Project ${randomUUID()}`,
        email,
        password,
      })
    );

    // Visit status so the user has an authenticator secret, then enroll in TOTP
    // without supplying a token.
    await request(app).get('/auth/mfa/status').set('Authorization', `Bearer ${accessToken}`);
  ...
  test('disable rejects a non-string token', async () => {
    const email = `disable-bad-token${randomUUID()}@example.com`;
    const password = 'password!@#';
    ...
    const { accessToken } = await withTestContext(() =>
      registerNew({
        firstName: 'Disable',
        lastName: 'BadToken',
        projectName: `Disable Bad Token Project ${randomUUID()}`,
        email,
        password,
      })
    );

    // Enroll in TOTP so the request passes the not-enrolled guard and reaches
    // the express-validator check.
    const statusRes = await request(app).get('/auth/mfa/status').set('Authorization', `Bearer ${accessToken}`);
    const secret = new URL(statusRes.body.enrollUri).searchParams.get('secret') as string;
    await request(app)
      .post('/auth/mfa/enroll')
    ...
    // A non-string token fails the `isString()` validator
    const res = await request(app)
      .post('/auth/mfa/disable')
      .set('Authorization', `Bearer ${accessToken}`)
      .type('json')
  ...
  test('verifyConnectedFactor returns false without a token', async () => {
    // The /disable endpoint guards against a missing token before calling this
    // helper, so exercise the defensive `!token` branch directly.
    const user = { resourceType: 'User', mfaMethod: ['totp'], mfaSecret: 'ABC' } as User;
    const login = { resourceType: 'Login' } as Login;
    expect(await verifyConnectedFactor(user, login, undefined)).toBe(false);
    expect(await verifyConnectedFactor(user, login, '')).toBe(false);
  });
  ...
  test('login-enroll enrolls in email-based MFA', async () => {
    ...
    const { accessToken, project, user } = await withTestContext(() =>
      registerNew({
        firstName: 'Login',
        lastName: 'EnrollEmail',
        projectName: `Login Enroll Email Project ${randomUUID()}`,
        email,
        password,
      })
    );
  ...
  test('login-enroll rejects an already-enrolled user', async () => {
    ...
    const { accessToken } = await withTestContext(() =>
      registerNew({
        firstName: 'Login',
        lastName: 'AlreadyEnrolled',
        projectName: `Login Already Enrolled Project ${randomUUID()}`,
        email,
        password,
      })
    );
    ...
    // A normal login (already enrolled) requires verification, not enrollment
    const loginRes = await request(app).post('/auth/login').type('json').send({ email, password, scope: 'openid' });
    expect(loginRes.body.mfaRequired).toBe(true);
  ...
  test('login-enroll requires a secret for TOTP enrollment', async () => {
    ...
    const { user } = await withTestContext(() =>
      registerNew({
        firstName: 'Login',
        lastName: 'NoSecret',
        projectName: `Login No Secret Project ${randomUUID()}`,
        email,
        password,
      })
    );
  ...
  test('login-enroll requires a token for TOTP enrollment', async () => {
    const email = `login-enroll${randomUUID()}@example.com`;
    const password = 'password!@#';
    ...
    const { accessToken, user } = await withTestContext(() =>
      registerNew({
        firstName: 'Login',
        lastName: 'NoToken',
        projectName: `Login No Token Project ${randomUUID()}`,
        email,
        password,
      })
    );

    // Visit status to ensure the user has an authenticator secret, then require MFA
    await request(app).get('/auth/mfa/status').set('Authorization', `Bearer ${accessToken}`);
    await setMfaRequired(user);
  ...
  test('login-enroll enrolls in TOTP with a valid token', async () => {
    const email = `login-enroll${randomUUID()}@example.com`;
    const password = 'password!@#';
    ...
    const { accessToken, user } = await withTestContext(() =>
      registerNew({
        firstName: 'Login',
        lastName: 'EnrollTotp',
        projectName: `Login Enroll Totp Project ${randomUUID()}`,
        email,
        password,
      })
    );
    ...
    const status = await request(app).get('/auth/mfa/status').set('Authorization', `Bearer ${accessToken}`);
    expect(status.body.enrolled).toBe(true);
    expect(status.body.enrolledMethods).toEqual(['totp']);
  });
