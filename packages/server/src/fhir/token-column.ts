...
export function buildTokenColumnsSearchFilter(
  ...
  switch (filter.operator) {
    ...
    case FhirOperator.CONTAINS: {
      ...
      const expressions = buildTokenColumnsWhereConditionTextAndContains(impl, tableName, filter.code, filter.operator, filter.value);
    ...
    case FhirOperator.NOT_EQUALS: {
      ...
      // https://www.hl7.org/fhir/r4/search.html#combining
      const expressions = buildTokenColumnsWhereConditionEqualsAndExact(impl, tableName, filter.code, filter.operator, filter.value);
...
function buildTokenColumnsWhereConditionEqualsAndExact(
  ...
  const condition = new Condition(new Column(tableName, impl.tokenColumnName), 'ARRAY_OVERLAPS', searchStrings, 'UUID[]');
