...
export const UUID_TEMPLATE = '00000000-0000-0000-0000-000000000000';
...
/**
 * Returns all `m` sentinel UUIDs used for array column padding.
 * These are generated by replacing the trailing digits of `UUID_TEMPLATE` with the index `0..m-1`.
 * @param m - The number of sentinel UUIDs to generate.
 * @returns An array of sentinel UUID strings.
 */
export function getAllPaddingSentinels(m: number): string[] {
  const sentinels: string[] = [];
  for (let i = 0; i < m; i++) {
    const indexStr = i.toString();
    if (indexStr.length > 12) {
      throw new Error('Array padding m too large');
    }
    sentinels.push(UUID_TEMPLATE.substring(0, UUID_TEMPLATE.length - indexStr.length) + indexStr);
  }
  return sentinels;
}

/**
 * Computes the padding fraction for a given array column padding configuration.
 * This is the probability that a row will receive a padding element.
 * @param config - The array column padding configuration.
 * @returns The padding fraction `(m * lambda) / (statisticsTarget * 300)`.
 */
export function getPaddingFraction(config: ArrayColumnPaddingConfig): number {
  return (config.m * config.lambda) / (config.statisticsTarget * 300);
}
