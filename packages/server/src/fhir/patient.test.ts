...
describe('FHIR Patient utils', () => {
  ...
  test('getPatientId', () => {
    expect(getPatientId({ resourceType: 'Account', subject: [] as Reference[] } as Account)).toBeUndefined();
    expect(getPatientId({ resourceType: 'Account', subject: [{}] } as Account)).toBeUndefined();
    expect(getPatientId({ resourceType: 'Account', subject: [{ reference: 'Device/123' }] } as Account)).toBeUndefined();
    expect(getPatientId({ resourceType: 'Account', subject: [{ reference: 'Patient/123' }] } as Account)).toBe('123');

    expect(getPatientId({ resourceType: 'Observation' } as Observation)).toBeUndefined();
    expect(getPatientId({ resourceType: 'Observation', subject: undefined } as Observation)).toBeUndefined();
    expect(getPatientId({ resourceType: 'Observation', subject: null } as any as Observation)).toBeUndefined();
    expect(getPatientId({ resourceType: 'Observation', subject: {} } as Observation)).toBeUndefined();
    expect(getPatientId({ resourceType: 'Observation', subject: { reference: 'Device/123' } } as Observation)).toBeUndefined();
    expect(getPatientId({ resourceType: 'Observation', subject: { reference: 'Patient/123' } } as Observation)).toBe('123');
