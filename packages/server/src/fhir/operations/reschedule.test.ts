...
describe('Appointment/:id/$reschedule', () => {
  ...
  // The project's own system repo rather than the global one: a project-typed resource
  // cannot be written through global-only routing where the database is sharded.
  ...
  test('leaves the service type on file, and the codes booked alongside it, untouched', async () => {
    // The visit type is an input to the move, never an output of it. `Appointment.serviceType`
    // carries the procedure codes a designated visit type is booked with, so rewriting it from
    // the parameter would quietly drop them — even for a move that changed nothing about the type.
    const practitionerSchedule = await makeSchedule(practitioner);
    const start = '2026-05-13T16:00:00.000Z'; // Wed 11am EST
    const end = '2026-05-13T17:00:00.000Z';
    const procedureCode: CodeableConcept = { coding: [{ system: 'http://www.ama-assn.org/go/cpt', code: '99213' }] };

    const booked = await book(makeProposal({ start, end, schedules: [practitionerSchedule] }), {
      serviceType: [...toServiceTypeCodeableConcepts(officeVisitService), procedureCode],
    });

    const response = await reschedule(booked.id as string, {
      start: '2026-05-14T16:00:00.000Z', // Thu 11am EST
      schedules: [practitionerSchedule],
    });

    expect(response).toHaveStatus(200);
    const appointments = bundleResources(response.body).filter((r) => isResource<Appointment>(r, 'Appointment'));
    expect(appointments[0].serviceType).toStrictEqual(booked.serviceType);
  ...
  test('rejects an appointment that records no service type at all', async () => {
    ...
    const untyped = await systemRepo.createResource<Appointment>({
      ...
      meta: { project: project.project.id },
      status: 'booked',
      start: '2026-05-27T16:00:00.000Z', // Wed 11am EST
      end: '2026-05-27T17:00:00.000Z',
      participant: [{ actor: createReference(patient), status: 'accepted' }],
    });

    const response = await reschedule(untyped.id, {
      start: '2026-05-28T16:00:00.000Z', // Thu 11am EST
      schedules: [practitionerSchedule],
    });
  ...
  test('deduplicates a schedule named more than once', async () => {
    const practitionerSchedule = await makeSchedule(practitioner);
    const roomOneSchedule = await makeSchedule(roomOne);
    const start = '2026-06-10T16:00:00.000Z'; // Wed 11am EST
    const end = '2026-06-10T17:00:00.000Z';
    const newStart = '2026-06-10T19:00:00.000Z'; // Wed 2pm EST

    const booked = await book(makeProposal({ start, end, schedules: [practitionerSchedule, roomOneSchedule] }));

    // Repeating a schedule holds its time once, the way $book's `uniqueOn` treats repeated
    // Slot schedule references
    const response = await reschedule(booked.id as string, {
      start: newStart,
      schedules: [practitionerSchedule, roomOneSchedule, practitionerSchedule],
    });
