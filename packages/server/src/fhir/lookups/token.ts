...
export class TokenTable extends LookupTable {
  ...
  buildWhere(
    ...
    const whereExpression =
      buildWhereExpression(impl, table, caseSensitive, filter) ??
      // TODO ARRAY_EMPTY is very untested
      new TypedCondition(new Column(table, impl.columnName), 'ARRAY_EMPTY', undefined);
...
function buildWhereCondition(
  ...
  if (parts.length === 2) {
    ...
    return buildValueCondition(impl, tableName, filter, caseSensitive, system || NULL_SYSTEM, value);
    // if (value) {
...
function buildValueCondition(
  ...
  if (impl.searchStrategy !== 'token-column') {
    throw new Error(`TokenTable.buildValueCondition not compatible with ${impl.searchStrategy}`);
  }
