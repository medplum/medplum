...
export class TokenTable extends LookupTable {
  ...
  buildWhere(
    ...
    if (impl.searchStrategy !== 'lookup-table') {
      throw new Error(`TokenTable.buildWhere not compatible with ${impl.searchStrategy}`);
    }
    ...
    const whereExpression = buildWhereExpression(impl, lookupTableName, caseSensitive, filter);
    if (whereExpression) {
...
export function addTokenColumnsOrderBy(
  ...
  */
  ...
  if (impl.searchStrategy !== 'token-column') {
    throw new Error(`addTokenColumnsOrderBy not compatible with ${impl.searchStrategy}`);
  }

  selectQuery.orderBy(
    new Column(
      undefined,
      `substring(a2t("${impl.columnName}"),` + escapeLiteral(ARRAY_DELIM + DELIM + '([^' + ARRAY_DELIM + ']+)') + ')',
      true
    ),
    sortRule.descending
  );
...
export function buildTokenColumnsSearchFilter(
  ...
  const impl = getSearchParameterImplementation(resourceType, param);
  if (impl.searchStrategy !== 'token-column') {
    throw new Error(`buildTokenColumnsSearchFilter not compatible with ${impl.searchStrategy}`);
  }
  ...
  if (subExpressions.length > 0) {
    ...
    // missing/present
    expression = new Negation(new TypedCondition(new Column(tableName, impl.columnName), 'ARRAY_EMPTY', undefined));
  }
...
function buildTokenColumnsWhereCondition(
  ...
  if (filter.operator === FhirOperator.IN) {
    ...
    return new TypedCondition(new Column(tableName, impl.columnName), 'ARRAY_CONTAINS_SUBQUERY', valueSetQ, 'TEXT[]');
  }
  ...
  if (shouldCompareTokenValue(filter.operator)) {
    return buildTokenColumnsValueCondition(impl, tableName, filter, caseSensitive, system, value);
  }
 ...
 * @param impl - The SearchParameterDetails for the token search parameter
...
function buildWhereExpression(
  impl: TokenColumnSearchParameterImplementation | LookupTableSearchParameterImplementation,
  tableName: string,
  caseSensitive: boolean,
  filter: Filter
): Expression | undefined {
  const subExpressions = [];
  for (const option of splitSearchOnComma(filter.value)) {
    const expression = buildWhereCondition(impl, tableName, filter, caseSensitive, option);
    if (expression) {
      subExpressions.push(expression);
    }
  }
...
function buildWhereCondition(
  ...
  if (parts.length === 2) {
    ...
    return value
      ? new Conjunction([systemCondition, buildValueCondition(impl, tableName, filter, caseSensitive, system, value)])
      : systemCondition;
  } else {
    // If using the :in operator, build the condition for joining to the ValueSet table specified by `query`
    if (filter.operator === FhirOperator.IN) {
    ...
    //value
    if (shouldCompareTokenValue(filter.operator)) {
      return buildValueCondition(impl, tableName, filter, caseSensitive, undefined, query);
    }
    // Otherwise we are just looking for the presence / absence of a token (e.g. when using the FhirOperator.MISSING)
    // so we don't need to construct a filter Condition on the token table.
    return undefined;
