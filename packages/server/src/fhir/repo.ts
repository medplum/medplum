...
export interface ResendSubscriptionsOptions extends InteractionOptions {
  ...
  subscription?: string;
  /**
   * Resend subscriptions for a specific historical version of the resource rather than the
   * current version. `%previous` is then the version immediately prior to it, which must be
   * within the first page of history results.
   */
  versionId?: string;
}
...
export class Repository extends FhirRepository implements Disposable {
  ...
  async resendSubscriptions<T extends Resource = Resource>(
    ...
    if (interaction === 'update') {
      // The previous version is the history entry immediately after the version being resent.
      // Without "versionId", that is the current version, which is always the first entry.
      const history = await this.readHistory<T>(resourceType, id, { limit: options?.versionId ? undefined : 2 });
      const index = options?.versionId
        ? (history.entry?.findIndex((e) => e.resource?.meta?.versionId === options.versionId) ?? -1)
        : 0;
      if (history.entry?.[index]?.resource?.meta?.versionId !== resource.meta?.versionId) {
        throw new OperationOutcomeError(preconditionFailed);
      }
      previousVersion = history.entry?.[index + 1]?.resource;
    }
