...
import { HISTORY_QUOTA_COST, READ_QUOTA_COST, SEARCH_QUOTA_COST, WRITE_QUOTA_COST } from './fhirquota';
...
export class Repository extends FhirRepository<PoolClient> implements Disposable {
  ...
  async recordFhirQuota(points: number): Promise<void> {
    ...
    if (ctx instanceof AuthenticatedRequestContext && ctx.isAsync) {
      ...
      const delay = points * (getConfig().asyncDelayScaling ?? 1);
  ...
  async createResource<T extends Resource>(resource: T, options?: CreateResourceOptions): Promise<WithId<T>> {
    await this.recordFhirQuota(WRITE_QUOTA_COST);
  ...
  async readResource<T extends Resource>(
    ...
    await this.recordFhirQuota(READ_QUOTA_COST);
  ...
  async readReferences<T extends Resource>(references: Reference<T>[]): Promise<(WithId<T> | Error)[]> {
    await this.recordFhirQuota(references.length * READ_QUOTA_COST);
  ...
  async readHistory<T extends Resource>(
    ...
    await this.recordFhirQuota(HISTORY_QUOTA_COST);
    const startTime = Date.now();
    try {
  ...
  async search<T extends Resource>(
    ...
    await this.recordFhirQuota(SEARCH_QUOTA_COST);
  ...
  async searchByReference<T extends Resource>(
    ...
    await this.recordFhirQuota(references.length * SEARCH_QUOTA_COST);
    const startTime = Date.now();
    try {
