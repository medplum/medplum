...
export class Repository extends BaseRepository implements FhirRepository {
  ...
  private canReadCacheEntry(cacheEntry: ResourceWrapper): boolean {
    if (
      !this.isSuperAdmin() &&
   ...
   */
  private async writeToDatabase<T extends Resource>(wrapper: ResourceWrapper<T>): Promise<void> {
   ...
   */
  private async reindexResourceImpl<T extends Resource>(wrapper: ResourceWrapper<T>): Promise<void> {
  ...
  async resendSubscriptions<T extends Resource>(resourceType: string, id: string): Promise<void> {
    ...
    const wrapper = await this.readResourceImpl<T>(resourceType, id);
    return addSubscriptionJobs(wrapper.resource as T, { interaction: 'update' });
  }
...
async function setCacheEntry(
  ...
  await getRedis().set(
    getCacheKey(projectId, resourceType, id),
 ...
 * @param ids The resource IDs.
 */
async function deleteCacheEntries(projectId: string | undefined, resourceType: string, ids: string[]): Promise<void> {
  const cacheKeys = ids.map((id) => {
    return getCacheKey(projectId, resourceType, id);
  });
