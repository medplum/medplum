 ...
 */
export class Repository<TClient extends PgClientLike = RepositoryDatabaseClient>
  extends FhirRepository
  implements Disposable
{
    ...
    try {
      ...
      if (!this.isCacheOnly(resource)) {
        await this.ensureInTransaction(async (txRepo) => {
          ...
          await (txRepo as this).deleteFromLookupTables(client, resource);
          const durationMs = Date.now() - startTime;
    ...
    const deletedIds = await this.withTransaction<string[]>(
      async (txRepo) => {
        ...
        for (let i = 0; i < deleteResult.length; i++) {
          ...
          deletedIds[i] = res.id;
          await (txRepo as this).deleteFromLookupTables(client, { resourceType, id: res.id } as WithId<Resource>);
        }
   ...
   */
  private async deleteFromLookupTables(client: PgClientLike, resource: WithId<Resource>): Promise<void> {
    for (const lookupTable of lookupTables) {
...
export class SystemRepository<TClient extends PgClientLike = RepositoryDatabaseClient> extends Repository<TClient> {}
 ...
 */
function createSystemRepository<TClient extends PgClientLike = RepositoryDatabaseClient>(
  shardId: string,
