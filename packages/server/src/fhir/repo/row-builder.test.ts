...
import type { Binary, DocumentReference, Observation } from '@medplum/fhirtypes';
...
describe('Repository row builder', () => {
  ...
  describe('Array column padding', () => {
    ...
    afterEach(() => {
      if (prevConfig) {
        getConfig().arrayColumnPadding = JSON.parse(prevConfig);
      } else {
        getConfig().arrayColumnPadding = undefined;
      }
    });
    ...
    test.each([
      ...
      withTestContext(async () => {
        if (arrayColumnPadding) {
          getConfig().arrayColumnPadding = arrayColumnPadding;
        }
        ...
        const results = await db.query('SELECT "__identifier" FROM "Observation" WHERE "id" = $1', [observation.id]);
        if (shouldPad) {
        ...
        await systemRepo.deleteResource(observation.resourceType, observation.id);
  ...
  describe('Array column value sorting', () => {
    test('stores multi-valued reference column in sorted order (DocumentReference.author)', () =>
      withTestContext(async () => {
        ...
        const document = await systemRepo.createResource<DocumentReference>({
        ...
        const db = getDatabasePool(DatabaseMode.READER);
        const results = await db.query('SELECT "author" FROM "DocumentReference" WHERE "id" = $1', [document.id]);
    ...
    test('stores multi-valued quantity column in sorted order (Observation.component)', () =>
      withTestContext(async () => {
        ...
        const db = getDatabasePool(DatabaseMode.READER);
        const results = await db.query('SELECT "componentValueQuantity" FROM "Observation" WHERE "id" = $1', [
          observation.id,
        ]);
