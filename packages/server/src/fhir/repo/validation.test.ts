...
describe('Repository validation', () => {
  ...
  test('Profile validation', () =>
    withTestContext(async () => {
      ...
      await expect(repo.createResource(patient)).rejects.toThrow(new Error('Missing required property (Patient.gender)'));
    }));
  ...
  test('Profile update', () =>
    withTestContext(async () => {
      ...
      await repo.updateResource<StructureDefinition>({
        ...
        snapshot: {
          ...profile.snapshot,
          element: profile.snapshot?.element?.map((element) => {
            if (element.path === 'Patient.address') {
              return {
                ...element,
                min: 1,
              };
      ...
      await expect(repo.createResource(patient)).rejects.toThrow(new Error('Missing required property (Patient.address)'));
    }));
  ...
  describe('Update resource with terminology validation', () => {
    ...
    beforeAll(async () => {
      ...
      const commLang = modifiedPatientProfile.snapshot?.element.find((element) => element.id === 'Patient.communication.language');
    ...
    test('Valid patient without any profiles', () =>
      withTestContext(async () => {
    ...
    test('Valid patient with US Core Patient profile', () =>
      withTestContext(async () => {
        await expect(repo.createResource({ ...patient, meta: { profile: [profile.url] } })).resolves.toBeDefined();
      }));

    test('Invalid patient with US Core Patient profile (communication.language not in ValueSet)', () =>
      withTestContext(async () => {
        await expect(
          repo.createResource({
            ...patient,
  ...
  test('Project default profiles', () =>
    withTestContext(async () => {
      const { repo } = await createTestProject({
        withRepo: true,
        project: {
          defaultProfile: [{ resourceType: 'Observation', profile: ['http://hl7.org/fhir/StructureDefinition/vitalsigns'] }],
        },
      });
