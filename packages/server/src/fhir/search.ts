...
export class RepositorySearch {
  ...
  async search<T extends Resource>(searchRequest: SearchRequest<T>): Promise<Bundle<T>> {
    this.validateSearchResourceTypes(searchRequest);
    ...
    if (searchRequest.count > 0) {
      ({ entry, rowCount, hasMore } = await this.getSearchEntries<T>(searchRequest));
    }

    let total = undefined;
    if (searchRequest.total === 'accurate' || searchRequest.total === 'estimate') {
      total = await this.getCount(searchRequest, rowCount);
    }
   ...
   */
  private validateSearchResourceTypes(searchRequest: SearchRequest): void {
    if (searchRequest.types) {
      for (const resourceType of searchRequest.types) {
        this.validateSearchResourceType(resourceType);
      }
    } else {
      this.validateSearchResourceType(searchRequest.resourceType);
    }
  }
   ...
   */
  private validateSearchResourceType(resourceType: ResourceType): void {
    ...
    if (!this.repo.canReadResourceType(resourceType)) {
  ...
  private async getSearchEntries<T extends Resource>(
    ...
    const builder = this.getBaseSelectQuery(searchRequest);
    ...
    const rows = await builder.execute(this.repo.getDatabaseClient(DatabaseMode.READER));
    ...
    if (searchRequest.include || searchRequest.revInclude) {
      await this.getExtraEntries(searchRequest, resources, entries);
    }
    ...
    for (const entry of entries) {
      ...
      this.removeResourceFields(entry.resource, searchRequest);
    }
  ...
  private getBaseSelectQuery(searchRequest: SearchRequest, addColumns = true): SelectQuery {
    let builder: SelectQuery;
    if (searchRequest.types) {
      const queries: SelectQuery[] = [];
      for (const resourceType of searchRequest.types) {
        queries.push(this.getBaseSelectQueryForResourceType(resourceType, searchRequest, addColumns));
      }
      ...
      builder = this.getBaseSelectQueryForResourceType(searchRequest.resourceType, searchRequest, addColumns);
    }
    return builder;
  }

  private getBaseSelectQueryForResourceType(
    ...
    this.repo.addDeletedFilter(builder);
    this.repo.addSecurityFilters(builder, resourceType);
    this.addSearchFilters(builder, resourceType, searchRequest);
    return builder;
  }

  private removeResourceFields(resource: Resource, searchRequest: SearchRequest): void {
    this.repo.removeHiddenFields(resource);
   ...
   */
  private async getExtraEntries<T extends Resource>(
  ...
  private async getSearchIncludeEntries(include: IncludeTarget, resources: Resource[]): Promise<BundleEntry[]> {
    ...
    const readResult = await this.repo.readReferences(references);
   ...
   */
  private async getCount(searchRequest: SearchRequest, rowCount: number | undefined): Promise<number> {
    const estimateCount = await this.getEstimateCount(searchRequest, rowCount);
    if (estimateCount < getConfig().accurateCountThreshold) {
      return this.getAccurateCount(searchRequest);
    }
  ...
  private async getAccurateCount(searchRequest: SearchRequest): Promise<number> {
    const builder = this.getBaseSelectQuery(searchRequest, false);
   ...
   * @returns The total number of matching results.
   */
  private async getEstimateCount(searchRequest: SearchRequest, rowCount: number | undefined): Promise<number> {
    const builder = this.getBaseSelectQuery(searchRequest);
   ...
   * @param searchRequest - The search request.
   */
  private addSearchFilters(selectQuery: SelectQuery, resourceType: ResourceType, searchRequest: SearchRequest): void {
    const expr = this.buildSearchExpression(selectQuery, resourceType, searchRequest);
    if (expr) {
    ...
    if (searchRequest.filters) {
      for (const filter of searchRequest.filters) {
        ...
        if (filter.code.startsWith('_has:') || filter.code.includes('.')) {
          const chain = this.parseChainedParameter(searchRequest.resourceType, filter.code, filter.value);
          expr = this.buildChainedSearch(selectQuery, searchRequest.resourceType, chain);
        } else {
          expr = this.buildSearchFilterExpression(selectQuery, resourceType, resourceType, filter);
        }
  ...
  private buildSearchFilterExpression(
    ...
    if (filter.code.startsWith('_has:') || filter.code.includes('.')) {
      const chain = this.parseChainedParameter(resourceType, filter.code, filter.value);
      return this.buildChainedSearch(selectQuery, resourceType, chain);
    }

    const specialParamExpression = this.trySpecialSearchParameter(selectQuery, resourceType, table, filter);
    ...
    return this.buildNormalSearchFilterExpression(resourceType, table, param, filter);
  }
   ...
   */
  private buildNormalSearchFilterExpression(
    resourceType: string,
    table: string,
    ...
    if (filter.operator === Operator.MISSING) {
      ...
      return this.buildStringSearchFilter(table, details, filter.operator, splitSearchOnComma(filter.value));
    } else if (param.type === 'token' || param.type === 'uri') {
      return this.buildTokenSearchFilter(table, details, filter.operator, splitSearchOnComma(filter.value));
    } else if (param.type === 'reference') {
      return this.buildReferenceSearchFilter(table, details, filter.operator, splitSearchOnComma(filter.value));
    } else if (param.type === 'date') {
      return this.buildDateSearchFilter(table, details, filter);
    } else if (param.type === 'quantity') {
      return new Condition(
        new Column(table, details.columnName),
        this.fhirOperatorToSqlOperator(filter.operator),
        filter.value
      );
    } else {
      const values = splitSearchOnComma(filter.value).map(
        (v) =>
          new Condition(new Column(undefined, details.columnName), this.fhirOperatorToSqlOperator(filter.operator), v)
      );
  ...
  private trySpecialSearchParameter(
    ...
    switch (filter.code) {
      ...
      case '_lastUpdated':
        return this.buildDateSearchFilter(
          table,
          { type: SearchParameterType.DATETIME, columnName: 'lastUpdated' },
          filter
        );
      case '_compartment':
      case '_project':
      ...
      case '_filter':
        return this.buildFilterParameterExpression(
          selectQuery,
          resourceType,
          table,
          parseFilterParameter(filter.value)
        );
      default:
        return undefined;
    }
  ...
  private buildFilterParameterExpression(
    ...
    if (filterExpression instanceof FhirFilterNegation) {
      return new Negation(
        this.buildFilterParameterExpression(selectQuery, resourceType, table, filterExpression.child)
      );
    } else if (filterExpression instanceof FhirFilterConnective) {
      return this.buildFilterParameterConnective(selectQuery, resourceType, table, filterExpression);
    } else if (filterExpression instanceof FhirFilterComparison) {
      return this.buildFilterParameterComparison(selectQuery, resourceType, table, filterExpression);
    } else {
  ...
  private buildFilterParameterConnective(
    ...
    const expressions = [
      this.buildFilterParameterExpression(selectQuery, resourceType, table, filterConnective.left),
      this.buildFilterParameterExpression(selectQuery, resourceType, table, filterConnective.right),
    ];
  ...
  private buildFilterParameterComparison(
    ...
    return this.buildSearchFilterExpression(selectQuery, resourceType, table, {
   ...
   */
  private buildStringSearchFilter(
    table: string,
    details: SearchParameterDetails,
    operator: Operator,
    values: string[]
    ...
    const expression = this.buildStringFilterExpression(column, operator, values);
    if (details.array) {
  ...
  private buildStringFilterExpression(column: Column, operator: Operator, values: string[]): Expression {
  ...
  private buildIdSearchFilter(
    ...
    const condition = this.buildEqualityCondition(details, values, column);
   ...
   * @returns The select query condition.
   */
  private buildTokenSearchFilter(
    table: string,
    details: SearchParameterDetails,
    operator: Operator,
    values: string[]
   ...
   * @returns The select query condition.
   */
  private buildReferenceSearchFilter(
    table: string,
    details: SearchParameterDetails,
    operator: Operator,
    values: string[]
   ...
   * @returns The select query condition.
   */
  private buildDateSearchFilter(table: string, details: SearchParameterDetails, filter: Filter): Expression {
   ...
   */
  private fhirOperatorToSqlOperator(fhirOperator: Operator): keyof typeof SQL {
    switch (fhirOperator) {
      case Operator.EQUALS:
        return '=';
      case Operator.NOT:
  ...
  private buildEqualityCondition(
    details: SearchParameterDetails,
    values: string[],
  ...
  private buildChainedSearch(
    ...
    if (this.usesReferenceLookupTable()) {
      return this.buildChainedSearchUsingReferenceTable(selectQuery, resourceType, param);
    } else {
      return this.buildChainedSearchUsingReferenceStrings(selectQuery, resourceType, param);
    }
  }

  private usesReferenceLookupTable(): boolean {
    return !!(
      getConfig().chainedSearchWithReferenceTables ||
      this.repo.currentProject()?.features?.includes('reference-lookups')
    );
  }
   ...
   */
  private buildChainedSearchUsingReferenceTable(
    selectQuery: SelectQuery,
    resourceType: string,
    param: ChainedSearchParameter
  ): Expression {
    ...
    for (const link of param.chain) {
      ...
      if (link.filter) {
        return this.buildSearchFilterExpression(
          selectQuery,
   ...
   * @returns The WHERE clause expression for the final chained filter.
   */
  private buildChainedSearchUsingReferenceStrings(
    selectQuery: SelectQuery,
    resourceType: string,
    param: ChainedSearchParameter
  ): Expression {
    ...
    for (const link of param.chain) {
      ...
      const joinCondition = this.buildSearchLinkCondition(currentResourceType, link, currentTable, nextTable);
      ...
      if (link.filter) {
        return this.buildSearchFilterExpression(selectQuery, link.resourceType as ResourceType, nextTable, link.filter);
      }
    }
    throw new OperationOutcomeError(badRequest('Unterminated chained search'));
  }

  private buildSearchLinkCondition(
    resourceType: string,
  ...
  private parseChainedParameter(resourceType: string, key: string, value: string): ChainedSearchParameter {
    const param: ChainedSearchParameter = {
      chain: [],
    };
    let currentResourceType = resourceType;

    const parts = this.splitChainedSearch(key);
    for (let i = 0; i < parts.length; i++) {
      const part = parts[i];
      if (part.startsWith('_has')) {
        const link = this.parseReverseChainLink(part, currentResourceType);
        ...
        const link = this.parseChainLink(part, currentResourceType);
        param.chain.push(link);
        currentResourceType = link.resourceType;
      }
    }
  ...
  private parseChainLink(param: string, currentResourceType: string): ChainedSearchLink {
  ...
  private parseReverseChainLink(param: string, targetResourceType: string): ChainedSearchLink {
  ...
  private splitChainedSearch(chain: string): string[] {
    const params: string[] = [];
    while (chain) {
