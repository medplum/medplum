// SPDX-FileCopyrightText: Copyright Orangebot, Inc. and Medplum contributors
// SPDX-License-Identifier: Apache-2.0

describe('toContainExactly', () => {
  test('passes when arrays contain the same elements in different order', () => {
    expect([1, 2, 3]).toContainExactly([3, 1, 2]);
    expect([{ a: 1 }, { b: 2 }]).toContainExactly([{ b: 2 }, { a: 1 }]);
  });

  test('fails when lengths differ', () => {
    expect(() => expect([1, 2]).toContainExactly([1, 2, 3])).toThrow(/length/);
    expect(() => expect([1, 2, 3]).toContainExactly([1, 2])).toThrow(/length/);
  });

  test('counts duplicates correctly', () => {
    expect([1, 1, 2]).toContainExactly([1, 2, 1]);
    expect(() => expect([1, 1, 2]).toContainExactly([1, 2, 2])).toThrow();
  });

  test('fails when received is not an array', () => {
    expect(() => expect('not an array').toContainExactly([])).toThrow(/array/);
  });

  test('supports asymmetric matchers inside expected', () => {
    expect([
      { name: 'a', value: 1 },
      { name: 'b', value: 2 },
    ]).toContainExactly([expect.objectContaining({ name: 'b' }), expect.objectContaining({ name: 'a' })]);
  });

  test('passes empty against empty', () => {
    expect([]).toContainExactly([]);
  });

  test('works as an asymmetric matcher via expect.toContainExactly', () => {
    expect({ wrapper: { arr: [3, 1, 2] } }).toEqual({
      wrapper: { arr: expect.toContainExactly([1, 2, 3]) },
    });
  });

  test('asymmetric matcher fails when length differs', () => {
    expect(() => expect({ arr: [1, 2, 3, 4] }).toEqual({ arr: expect.toContainExactly([1, 2, 3]) })).toThrow();
  });

  test('negated form passes when arrays differ', () => {
    expect([1, 2, 3]).not.toContainExactly([1, 2, 4]);
    expect([1, 2]).not.toContainExactly([1, 2, 3]);
    expect('not an array').not.toContainExactly([]);
  });

  test('negated form fails when arrays match unordered', () => {
    expect(() => expect([1, 2, 3]).not.toContainExactly([3, 1, 2])).toThrow(/not to equal/);
  });

  test('failure message includes expected and received arrays', () => {
    expect(() => expect([1, 2, 3]).toContainExactly([1, 2, 4])).toThrow(/Expected[\s\S]*Received/);
  });

  test('inverse asymmetric matcher passes when arrays differ', () => {
    expect({ arr: [1, 2, 3] }).toEqual({ arr: expect.not.toContainExactly([1, 2, 4]) });
  });

  test('inverse asymmetric matcher fails when arrays match unordered', () => {
    expect(() => expect({ arr: [3, 1, 2] }).toEqual({ arr: expect.not.toContainExactly([1, 2, 3]) })).toThrow();
  });
});

describe('toHaveStatus', () => {
  test('passes when status matches', () => {
    expect({ status: 200, body: {}, text: '' }).toHaveStatus(200);
  });

  test('fails with response body in message when status differs', () => {
    expect(() =>
      expect({ status: 400, body: { issue: [{ details: { text: 'Bad request' } }] }, text: '' }).toHaveStatus(200)
    ).toThrow(/Expected status 200, received 400[\s\S]*Bad request/);
  });

  test('falls back to response text when body is not serializable', () => {
    const body: Record<string, unknown> = {};
    body.self = body;
    expect(() => expect({ status: 500, body, text: 'raw text' }).toHaveStatus(200)).toThrow(/raw text/);
  });

  test('negated form passes when status differs', () => {
    expect({ status: 404, body: {}, text: '' }).not.toHaveStatus(200);
  });

  test('negated form fails when status matches', () => {
    expect(() => expect({ status: 200, body: {}, text: '' }).not.toHaveStatus(200)).toThrow(/not to be 200/);
  });
});
