...
describe('Reindex Worker', () => {
  ...
  test('Reindex resource type with empty page', () =>
    withTestContext(async () => {
      ...
      await addReindexJob(GLOBAL_SHARD_ID, ['MedicinalProductManufactured'], asyncJob);
      ...
      await reindexJob.execute(undefined);
  ...
  test('prepare job', async () => {
    ...
    const jobData1 = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, ['Patient']);
    ...
    await withTestContext(() => {
      const jobData2 = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, ['Patient']);
  ...
  test('Proceeds to next resource type after exhausting initial one', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, resourceTypes);
  ...
  test('Fails job on error', () =>
    withTestContext(async () => {
      ...
      await withQueryInterceptor(
        ...
        async () => {
          const jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, ['ValueSet'], { maxIterationAttempts: 1 });
          const reindexJob = await ReindexJob.create(jobData);
  ...
  test('Retries iteration on error when maxIterationAttempts > 1', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, ['PaymentNotice'], {
  ...
  test('Fails after exhausting maxIterationAttempts', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, ['ValueSet'], {
  ...
  test('Rethrows after exhausting maxIterationAttempts with thrown exceptions', () =>
    withTestContext(async () => {
      ...
      const originalLevel = globalLogger.level;
      globalLogger.level = LogLevel.NONE;
      await expect(reindexJob.execute(undefined)).rejects.toThrow('Persistent thrown exception');
      globalLogger.level = originalLevel;
  ...
  test('Throws error when maxIterationAttempts is less than 1', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, ['PaymentNotice'], { maxIterationAttempts: 0 });
  ...
  test('Handles DEFAULT upsertStatementTimeout without failing', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, ['MedicinalProductManufactured']);
  ...
  test('Reindex with search filter', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, resourceTypes, { searchFilter });
  ...
  test.each([
    ...
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob }, ['Patient'], {
...
describe('Job cancellation', () => {
  ...
  test('Ensure job reads up-to-date cancellation status from DB', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob: originalJob }, ['MedicinalProductContraindication']);
  ...
  test.each(['some-token', undefined])('Job handles queue closing with job.token %s', async (jobToken) =>
    withTestContext(async () => {
      ...
      try {
        await (await ReindexJob.create(jobData)).execute(job);
  ...
  test.each([
    ...
    async (minReindexWorkerVersion, jobToken) => {
      ...
      await withTestContext(async () => {
        jobData = prepareReindexJobData({ shardId: GLOBAL_SHARD_ID, asyncJob: originalJob }, ['MedicinalProductContraindication']);
      });
      ...
      const result = await (await ReindexJob.create(jobData)).execute(job);
  ...
  test('Throws error when updating AsyncJob output fails with non-412 error', () =>
    withTestContext(async () => {
      const asyncJob = await repo.createResource<AsyncJob>({
        resourceType: 'AsyncJob',
        status: 'accepted',
      ...
      vi.spyOn(Repository.prototype, 'updateResource').mockRejectedValue(testError);

      const reindexJob = await ReindexJob.create(jobData);
      await expect(reindexJob.execute(undefined)).rejects.toThrow('Database connection failed');
    }));
});
