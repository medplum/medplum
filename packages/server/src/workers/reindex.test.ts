...
describe('Reindex Worker', () => {
  ...
  test('Reindex resource type with empty page', () =>
    withTestContext(async () => {
      ...
      await addReindexJob(repo.shardId, ['MedicinalProductManufactured'], asyncJob);
  ...
  test('prepare job', async () => {
    ...
    const jobData1 = prepareReindexJobData(repo.shardId, ['Patient'], 'asyncJobId1');
    ...
    withTestContext(() => {
      const jobData2 = prepareReindexJobData(repo.shardId, ['Patient'], 'asyncJobId1');
  ...
  test('Multiple iterations when more than one batchSize exist', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData(repo.shardId, ['ImmunizationEvaluation'], asyncJob.id, {
  ...
  test('Proceeds to next resource type after exhausting initial one', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData(repo.shardId, resourceTypes, asyncJob.id);
  ...
  test('Fails job on error', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData(repo.shardId, ['ValueSet'], asyncJob.id, { maxIterationAttempts: 1 });
  ...
  test('Retries iteration on error when maxIterationAttempts > 1', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData('test-shard-id', ['PaymentNotice'], asyncJob.id, {
  ...
  test('Handles DEFAULT upsertStatementTimeout without failing', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData('test-shard-id', ['MedicinalProductManufactured'], asyncJob.id);
  ...
  test('Reindex with search filter', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData(repo.shardId, resourceTypes, asyncJob.id, { searchFilter });
  ...
  test.each([
    ...
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData(repo.shardId, ['Patient'], asyncJob.id, {
...
describe('Job cancellation', () => {
  ...
  test('Detect cancelled AsyncJob when iteration begins', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData(repo.shardId, ['MedicinalProductContraindication'], asyncJob.id);
  ...
  test('Ensure job reads up-to-date cancellation status from DB', () =>
    withTestContext(async () => {
      ...
      const jobData = prepareReindexJobData(repo.shardId, ['MedicinalProductContraindication'], originalJob.id);
  ...
  test.each([
    ...
    async (minReindexWorkerVersion, jobToken) => {
      ...
      await withTestContext(async () => {
        jobData = prepareReindexJobData(repo.shardId, ['MedicinalProductContraindication'], originalJob.id);
      });
