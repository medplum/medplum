...
describe('Subscription Worker', () => {
  ...
  test('WebSocket Subscription -- Subscription Has No Author', () =>
    withTestContext(async () => {
      globalLogger.level = LogLevel.WARN;
      const originalConsoleLog = console.log;
      console.log = jest.fn();
      ...
      const { repo: wsRepo, membership } = await createTestProject({
        ...
        withRepo: true,
        project: {
          name: 'WebSockets AccessPolicy No Author Project',
          features: ['websocket-subscriptions'],
        },
        membership: {
          accessPolicy: createReference(accessPolicy),
      ...
      const updated = await superAdminRepo.updateResource({ ...subscription, meta: { author: undefined } });
      expect(updated.meta?.author).not.toBeDefined();
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('[Subscription Access Policy]: No author for subscription')
      );
      console.log = originalConsoleLog;
      globalLogger.level = LogLevel.NONE;
    }));
