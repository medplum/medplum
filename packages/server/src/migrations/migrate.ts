...
export interface GenerateMigrationResult {
  preDeployActions: MigrationAction[];
  postDeployActions: MigrationAction[];
  postDeployDescriptions: string[];
}
...
/**
 * Generates migration actions with pre-deploy and post-deploy actions separated.
 * This is the new preferred way to generate migrations as it allows for better
 * user interaction and automatic post-deploy migration file generation.
...
export async function generateSeparatedMigrationActions(
  ...
  for (const startTable of startDefinition.tables) {
    if (!matchedStartTables.has(startTable)) {
      const description = `DROP TABLE ${escapeMixedCaseIdentifier(startTable.name)}`;
      ctx.collectPostDeployAction(() => ({ type: 'DROP_TABLE', tableName: startTable.name }), description);
    }
  }

  if (options.analyzeResourceTables) {
    for (const resourceType of getResourceTypes()) {
      preDeployActions.push({ type: 'ANALYZE_TABLE', tableName: resourceType });
    }
  }

  return { preDeployActions, postDeployActions, postDeployDescriptions };
}

async function buildStartDefinition(options: { dbClient: DbClient }): Promise<SchemaDefinition> {
...
function buildCodingTable(result: SchemaDefinition): void {
  result.tables.push({
    ...
    indexes: [
      ...
      { columns: ['foobar'], indexType: 'btree' },
    ],
  });
}
...
/**
 * Writes post-deploy migration actions to a FileBuilder in the CustomPostDeployMigration format.
 * This generates a file compatible with the existing v28.ts pattern.
 * @param b - The FileBuilder to write to.
 * @param actions - The migration actions to write.
 */
export function writePostDeployActionsToBuilder(b: FileBuilder, actions: MigrationAction[]): void {
  b.append('// Generated by @medplum/generator');
  ...
  for (const action of actions) {
    switch (action.type) {
      ...
      case 'ADD_CONSTRAINT': {
        b.appendNoWrap(
          `await fns.nonBlockingAddCheckConstraint(client, results, '${action.tableName}', '${action.constraintName}', \`${action.constraintExpression}\`);`
        );
        break;
      }
      default: {
        // Pre-deploy only actions should not appear in post-deploy migrations
        throw new Error(`Unsupported post-deploy action type: ${(action as MigrationAction).type}`, { cause: action });
      }
    }
  }
...
function generateColumnsActionsWithSeparation(
  ...
  for (const startColumn of startTable.columns) {
    if (!targetTable.columns.some((c) => c.name === startColumn.name)) {
      ctx.collectPostDeployAction(
        () => ({ type: 'DROP_COLUMN', tableName: targetTable.name, columnName: startColumn.name }),
    ...
    if (startIndex) {
      ...
      ctx.collectPostDeployAction(
        () => {
          ...
          return { type: 'CREATE_INDEX', indexName, createIndexSql };
        },
  ...
  for (const startIndex of startTable.indexes) {
    if (!matchedIndexes.has(startIndex)) {
      ...
      if (options?.dropUnmatchedIndexes) {
        ...
        preDeploy.push({ type: 'DROP_INDEX', indexName });
      }
    }
  }
  return { preDeploy };
