...
export function parseIndexDefinition(indexdef: string): IndexDefinition {
  ...
  const parsedExpressions = parseIndexColumns(matches[1]);
  const columns = parsedExpressions.map<IndexDefinition['columns'][number]>((expression, i) => {
    ...
    if (!name) {
      ...
      // name here. If we want to be stricter and match on index name as well, throw an error here
      name = 'placeholder';
    }
...
function getSearchParameterIndexes(impl: SearchParameterImplementation): IndexDefinition[] {
