...
import type { BuildMigrationOptions, GenerateMigrationResult } from './migrate';
import {
  buildMigration,
  buildSchema,
...
function parseCliOptions(): CliOptions {
  ...
  const options: CliOptions = {
    ...
    // Deprecated
    skipPostDeploy: args.includes('--skipPostDeploy'),
    allowPostDeploy: args.includes('--allowPostDeploy'),
  };

  // Handle deprecated flags with warnings
  if (options.skipPostDeploy) {
    console.warn('\x1b[33m⚠️  Warning: --skipPostDeploy is deprecated. Use --preDeployOnly instead.\x1b[0m');
    options.preDeployOnly = true;
  }

  if (options.allowPostDeploy) {
    console.warn(
      '\x1b[33m⚠️  Warning: --allowPostDeploy is deprecated. Post-deploy migrations are now generated by default.\x1b[0m'
    );
  }
...
export async function main(): Promise<void> {
  ...
  // Handle schema writing (doesn't need DB)
  ...
  // Use legacy flow if deprecated flags are used
  if (options.skipPostDeploy || options.allowPostDeploy) {
    await runLegacyMigration(options);
    return;
  }

  // New flow: generate separated migrations
  await runSeparatedMigration(options);
}

async function runLegacyMigration(options: CliOptions): Promise<void> {
  ...
  const buildOptions: BuildMigrationOptions = {
    dbClient,
    skipPostDeployActions: options.preDeployOnly,
    allowPostDeployActions: options.allowPostDeploy,
  ...
  await buildMigration(b, buildOptions);
  ...
  if (options.dryRun) {
    ...
    console.log(`✓ Pre-deploy migration written to v${getNextSchemaVersion() - 1}.ts`);
  }
}

async function runSeparatedMigration(options: CliOptions): Promise<void> {
  const dbClient = new Client({
    host: 'localhost',
    port: 5432,
    database: 'medplum',
