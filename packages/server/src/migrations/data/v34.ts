...
async function callback(client: PoolClient, results: MigrationActionResult[]): Promise<void> {
  await fns.idempotentCreateIndex(client, results, 'HealthcareService_offeredIn_idx', `CREATE INDEX CONCURRENTLY IF NOT EXISTS "HealthcareService_offeredIn_idx" ON "HealthcareService" USING gin ("offeredIn")`);
}
