...
async function callback(client: PoolClient, results: MigrationActionResult[]): Promise<void> {
  await fns.idempotentCreateIndex(client, results, 'Observation_subject_date_idx', `CREATE INDEX CONCURRENTLY IF NOT EXISTS "Observation_subject_date_idx" ON "Observation" ("subject", "date")`);
}
