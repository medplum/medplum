...
async function callback(client: PoolClient, results: MigrationActionResult[]): Promise<void> {
  ...
  // Hand-added: the index above supersedes the plain trigram index on display, whose predicate
  // (`display ILIKE`) no longer has any caller. Dropped after the replacement is live so filtered
  // $expand never runs without a usable index, and before the backfill below so the rewrite only has
  // one GIN index to maintain.
