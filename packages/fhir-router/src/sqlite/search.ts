...
export async function searchByReferenceImpl<T extends Resource>(
  ...
  const rows: {
    content: string;
    ref: string;
  }[] = await unionAllBuilder.execute(repo.getSqlConnection());
 ...
 */
function validateSearchResourceTypes(repo: SqliteSearchRepo, searchRequest: SearchRequest): void {
 ...
 */
function validateSearchResourceType(repo: SqliteSearchRepo, resourceType: ResourceType): void {
...
async function getSearchEntries<T extends Resource>(
  ...
  const client = repo.getSqlConnection();
...
function removeResourceFields(resource: Resource, repo: SqliteSearchRepo, searchRequest: SearchRequest): void {
...
function getSearchUrl(searchRequest: SearchRequest): string {
  return `https://example.com/fhir/R4/${searchRequest.resourceType}${formatSearchQuery(searchRequest)}`;
}
...
export async function getCount(
  ...
  const shouldGetAccurate = options?.forceAccurate || estimate < 1000;
 ...
 */
async function getAccurateCount(repo: SqliteSearchRepo, searchRequest: SearchRequest): Promise<number> {
  ...
  const rows = await builder.execute(repo.getSqlConnection());
 ...
 * @returns The total number of matching results.
 */
async function getEstimateCount(repo: SqliteSearchRepo, searchRequest: SearchRequest): Promise<number> {
 ...
 * @param searchRequest - The search request.
 */
function addSortRules(repo: SqliteSearchRepo, builder: SelectQuery, searchRequest: SearchRequest): void {
