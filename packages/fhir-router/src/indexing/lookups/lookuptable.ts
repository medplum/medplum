...
export abstract class LookupTable {
   ...
   */
  indexResource(client: SqlConnection, resource: WithId<Resource>, create: boolean): Promise<void> {
   ...
   */
  async deleteValuesForResource(client: SqlConnection, resource: WithId<Resource>): Promise<void> {
  ...
  async batchDeleteValuesForResources<T extends Resource>(client: SqlConnection, resources: T[]): Promise<void> {
  ...
  protected static async purge(client: SqlConnection, lookupTableName: string, ...joins: TableJoin[]): Promise<void> {
