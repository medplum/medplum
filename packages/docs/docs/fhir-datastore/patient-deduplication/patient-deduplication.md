...
Batch pipelines run as offline jobs that consider all records at once to produce pairs of patient matches. Most implementations schedule these pipelines to run on a regularly scheduled interval. As this is an N<sup>2</sup> problem, they are primarily constrained by memory rather than latency.
