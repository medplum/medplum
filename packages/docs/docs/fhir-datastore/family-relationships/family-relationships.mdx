...
For maternity applications where the same patient can have multiple pregnancies over time, usually keep one `Patient` resource for the person and create one [`EpisodeOfCare`](/docs/api/fhir/resources/episodeofcare) per pregnancy. Link pregnancy-specific encounters, observations, care plans, tasks, and communications to the active episode when the resource supports that relationship. Use one `Patient` per pregnancy only when downstream systems require each pregnancy to behave as a separate chart, and keep an external identity link so those charts still map to the same authenticated person.
