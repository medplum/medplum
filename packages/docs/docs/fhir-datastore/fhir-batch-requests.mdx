...
Synchronous batch requests are subject to an 8 MB request size limit (Medplum-hosted) and a ~1 minute timeout. If you need to exceed these constraints, Medplum offers asynchronous batch requests. There are two main reasons you may want to process a batch asynchronously:

1. **Large or slow bundles** — Bundles that exceed the 1 MB size limit or take longer than ~1 minute to process will fail mid-request. Async raises the size limit to 50 MB and removes the timeout by enqueuing the bundle for background processing.
