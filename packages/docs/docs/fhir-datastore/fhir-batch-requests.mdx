...
For a full step-by-step (status URL and `Content-Location`, polling `GET …/job/{id}/status`, reading the results `Binary`, access policies, and edge cases), see **[Processing asynchronous bundles](/docs/fhir-datastore/processing-async-bundles)**.
