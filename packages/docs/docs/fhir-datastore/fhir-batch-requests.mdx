...
**Note:** When using bundle entries, the `ifMatch` property uses the ETag format: `W/"versionId"` (e.g., `W/"${patient.meta?.versionId}"`). This is different from the SDK's `updateResource` method which uses just the `versionId` string.
