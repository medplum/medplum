...
1. **Large or slow bundles** — Bundles that exceed the 8 MB size limit or take longer than ~1 minute to process will fail mid-request. Async raises the size limit to 50 MB and removes the timeout by enqueuing the bundle for background processing.
