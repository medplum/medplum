...
The behavior of the `Patient.identifier` field is important to note. `Patient.identifier` usually has a reference string or URL that describes which system that identifier came from. [`Identifiers`](/docs/fhir-basics#naming-data-identifiers) are a concept in FHIR which describe the context in which that identifier is generated. For example, an identifier could be a Social Security Number (SSN) or be created by a health system for its own internal purposes. Here is an example of an identifier scheme for the [Australian Healthcare system](https://namespaces.digitalhealth.gov.au/id/hi/ihi/1.0/).
