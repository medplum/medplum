...
Provide the current version of the resource and a search query, similar to `createResourceIfNoneExists()`. If the
...
<details>
  <summary>Example: Upsert a Patient record</summary>
