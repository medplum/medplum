...
A charting UI reads patient-level data that persists across visits and encounter-level data created during each visit. This page consolidates the FHIR resources and patterns used to populate patient summaries, sidebar lists, and encounter documentation. For visit orchestration and SOAP-style workflow, see [Visit Templates](/docs/charting/visit-templates).
...
Some demographic fields need careful handling. Summarized below.
...
/* Add observations with sampler.addObservation(...) then call sampler.summarize(...) */
```
