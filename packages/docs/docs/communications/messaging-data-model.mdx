...
| `category` | The type of message being conveyed, like a tag applied to the thread or message. | [HL7 Communication Category](http://terminology.hl7.org/CodeSystem/communication-category), SNOMED, custom | [See below](#how-to-tag-or-group-threads) |
...
| --- | --- |
| Draft | `Communication.status` is `preparation`. See [Message Editing and Drafts](/docs/communications/message-editing-and-drafts). |
...
| Retracted | `Communication.status` is `entered-in-error` for retract-and-correct or similar workflows. See [Message Editing and Drafts](/docs/communications/message-editing-and-drafts). |
...
Put `category` on both the thread header and child [`Communication`](/docs/api/fhir/resources/communication) resources. `category` is an array, so each resource can carry multiple tags.
