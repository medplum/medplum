...
How you populate `sender` and `recipient` on the **thread header** affects these queries—for recommended modeling (including listing the thread creator in `recipient`), see [Organizing Communications — Building and Structuring Threads](/docs/communications/organizing-communications#building-and-structuring-threads).
...
| Threads I participate in    | Add `&recipient=Practitioner/{id}`                           | Header must list the creator in `recipient`; see [note below](#filter-by-current-user) and [modeling tip](/docs/communications/organizing-communications#building-and-structuring-threads) |
...
For how to model thread headers so this works, see the [thread header `sender` / `recipient` recommendation](/docs/communications/organizing-communications#building-and-structuring-threads) in Organizing Communications.
