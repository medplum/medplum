...
- No → Use [Option B: Extension on thread header](#option-b-extension-on-thread-header). Not searchable, but works for groups; you can still show a "user up to date" dot and sort by timestamps.
...
Limitation: Not searchable via API — you cannot run a query like "all threads where Practitioner X has unread messages." You can still, for a thread you already have, read the extension and compare to the latest message to show a "user is up to date" dot and sort the thread list by last message timestamp.
...
<Tabs groupId="language">
  <TabItem value="ts" label="TypeScript">
    <MedplumCodeBlock language="ts" selectBlocks="patchThreadReadStateTs">
      {ExampleCode}
    </MedplumCodeBlock>
  </TabItem>
</Tabs>
...
Con: Most complex data model; requires creating and updating Task resources (typically via a [Bot](/docs/bots/bot-basics) on message create and when the user marks as read).

### Create a Read-Receipt Task When a Message is Sent

Typically done in a Bot triggered by a [Subscription](/docs/subscriptions) on [`Communication`](/docs/api/fhir/resources/communication) create. Use `Task.code` to distinguish read-receipt Tasks from other Task types (e.g. response-routing).

<Tabs groupId="language">
  <TabItem value="ts" label="TypeScript">
    <MedplumCodeBlock language="ts" selectBlocks="createReadReceiptTaskTs">
      {ExampleCode}
    </MedplumCodeBlock>
  </TabItem>
</Tabs>
...
- [Organizing Communications Using Threads](/docs/communications/organizing-communications) — thread structure and Communication elements
- [Message Response Tracking and Routing Using Tasks](/docs/communications/task-based-message-response-tracking-and-routing) — using Tasks for assignment and routing (distinct from read receipts)
