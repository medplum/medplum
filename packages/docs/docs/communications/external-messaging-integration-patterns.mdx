...
tags: [communications, messaging, bots, subscriptions, SMS, integration]
keywords:
  [
    Communication,
    Subscription,
...
These patterns show how to bridge Medplum messaging to external channels (SMS, email, push notifications) using [Bots](/docs/bots/bot-basics) and [Subscriptions](/docs/subscriptions). For in-platform automations (Tasks, reminders, scheduling), see [Messaging Automations](/docs/communications/messaging-automations).

:::note Prerequisites

Bots and Subscriptions are [advanced features](/docs/bots/bot-basics) that may need to be enabled for your project.
...
```mermaid
flowchart LR
    CreateComm["Provider sends<br/>'Your lab results are ready'"] --> Sub["FHIR Subscription<br/>(on new message)"]
    Sub --> Bot["Medplum Bot<br/>(notification handler)"]
    Bot --> External["SMS to patient<br/>via Twilio (example)"]
```

Create a Bot that sends an external notification when a child `Communication` is created:

<MedplumCodeBlock language="ts" selectBlocks="externalNotifyOnMessageBotTs">
  {ExampleCode}
</MedplumCodeBlock>

Create a `Subscription` to invoke that Bot for new messages in threads:

<MedplumCodeBlock language="ts" selectBlocks="externalNotifySubscriptionTs">
  {ExampleCode}
</MedplumCodeBlock>

The criteria `part-of:missing=false` limits matching resources to child messages (which have `partOf`), not thread headers. `status=in-progress` avoids firing on retracted messages (`entered-in-error`) or drafts (`preparation`).
...
Create the thread header once (no `partOf`, no `payload` on the header):
...
When the patient reached you on SMS, provider replies composed in your app should often go back out on SMS. After you create the thread header with `medium` indicating SMS (as in Strategy 1), new child `Communication` resources from providers can trigger the same outbound Subscription as in [Outbound: Notify External Systems on New Messages](#outbound-messaging-notifications). The Bot inspects the thread header: if `medium` includes SMS, resolve the patient from `subject`, read `Patient.telecom` for a phone number, and call your SMS API.

```mermaid
flowchart LR
    PatientSms["Patient sends SMS"] --> InboundBot["Inbound Bot creates Communication"]
    InboundBot --> Thread["Thread header with SMS medium"]
    ProviderReply["Provider sends in-app reply"] --> Sub["Subscription on new message"]
    Sub --> OutBot["Outbound Bot reads header.medium"]
    OutBot --> SmsOut["SMS to patient.telecom"]
```
...
Use the same Subscription criteria as the generic outbound notifier (`Communication?part-of:missing=false&status=in-progress`), or deploy a dedicated Bot that only runs the round-trip branch. Your product may also key off the last inbound message’s `medium` instead of the header if that fits your model better.
