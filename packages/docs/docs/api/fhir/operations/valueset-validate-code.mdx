...
properties can be required to have their values drawn from a given set of values specified by a [`ValueSet`](/docs/api/fhir/resources/valueset),
which may need to be [expanded](/docs/api/fhir/operations/valueset-expand) behind the scenes.
...
**Example**: Specified `ValueSet` could not be found by URL
