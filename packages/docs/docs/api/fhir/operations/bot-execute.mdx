...
To run bots asynchronously, you can specify the `Prefer: respond-async` header to move execution of the bot to the background.  Asynchronous execution will result in an HTTP `202 Accepted` response immediately.
