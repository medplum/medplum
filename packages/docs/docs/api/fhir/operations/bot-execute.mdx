...
You can poll this URL to check the status of your bot execution:
...
async function checkJobStatus(statusUrl: string): Promise<any> {
  ...
  if (asyncJob.status === 'completed') {
    ...
    const responseBody = asyncJob.output?.parameter?.find((p) => p.name === 'responseBody');
    return responseBody.valueString || responseBody.valueBoolean || responseBody.valueInteger;
  } else if (asyncJob.status === 'error') {
    // Handle error case
    const outcome = asyncJob.output?.parameter?.find((p) => p.name === 'outcome');
