---
sidebar_label: State-by-State Licensure
sidebar_position: 12
tags: [scheduling]
---

# State-by-State Licensure for Each Healthcare Service

:::info Scheduling operations do not enforce licensure

Use cases like state-by-state licensure, program enrollment, and “which providers may this patient book?” are **outside** Medplum’s scheduling operations. [`$find`](/docs/scheduling/schedule-find) does not take a patient, a license, or a state; it only computes free slots for a **single** [Schedule](/docs/api/fhir/resources/schedule) you already identified.
...
A single [Practitioner](/docs/api/fhir/resources/practitioner) may deliver more than one kind of care, modeled as multiple [HealthcareService](/docs/api/fhir/resources/healthcareservice) offerings (for example registered nurse care management and dietitian services). Licensure is usually **per service and per jurisdiction**: the same person might be licensed as an RN in California and Maryland, and as a dietitian in Ohio and Oregon.
...
### Model each enrollable service as a PractitionerRole
