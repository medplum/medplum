...
You still need a clear way to answer _who is allowed to provide this service to this patient in this state?_—but that answer is **input to Schedule selection**, not something `$find` infers for you.
...
```json
{
  ...
  "location": [{ "reference": "Location/us-ca" }, { "reference": "Location/us-md" }],
  "active": true
}
```
