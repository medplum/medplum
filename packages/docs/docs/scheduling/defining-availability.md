...
| `timezone`          | Code                                          | _none_                                      | Specifies the timezone (IANA timezone identifier, e.g., `America/New_York`) for interpreting availability. When not set, falls back to the `Schedule.actor`'s timezone. | Not recommended; set on `Schedule` or `Schedule.actor` instead |                                                           |
