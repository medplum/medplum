...
`package` includes commands for building and publishing Medplum marketplace packages.

A package is described by a `manifest.ts` in the package directory, which default-exports a `manifest` const built with `defineManifest` from [`@medplum/package-types`](https://www.npmjs.com/package/@medplum/package-types). The manifest declares the bots the package implements, how consumers reach them, and any reference data it ships.

:::caution[]

The `package` commands are in beta, and likely to change.

:::

#### validate

Validates a manifest without contacting a server. Checks both the module source (a manifest must be declarative: no top-level side effects, and only `import type` beyond `@medplum/package-types`) and the manifest's semantics (semver shape, identifier uniqueness, runtime matrix completeness, and every consumer-to-impl cross reference).

```bash
medplum package validate [packageDir]
```

`packageDir` defaults to the current directory.

##### optional flags for `package validate`

- `--existing-versions <versions>`
  - Comma-separated list of versions already published for this package
  - Any collision with the manifest's `version` is reported as an error

#### build

Compiles a manifest into the artifacts an install consumes: the install `Bundle` written into the customer project, and the impl `Bot` specs deployed into the package's implementation project. Validation runs first, so a build implies a `validate`.

```bash
medplum package build [packageDir]
```

Writes `install-bundle.json`, `impl-bots.json`, and `build-manifest.json` to the output directory.

##### optional flags for `package build`

- `-t, --target <target>`

  - Deployment target the manifest is compiled for
  - Choices are `local`, `staging`, and `production`
  - Defaults to `staging`

- `-o, --out <dir>`
  - Output directory
  - Defaults to `<packageDir>/dist/package/<target>`

#### publish

Publishes a package to a Medplum server: creates or reuses the implementation project, deploys the impl bots into it, uploads the install Bundle as a `Binary`, and registers an immutable `PackageRelease` in the catalog.

```bash
medplum package publish [packageDir] --apply
```

Publishing requires super-admin credentials. It runs as a dry run unless `--apply` is passed, because a real publish deploys bot code into a shared project and registers a release that cannot be rewritten afterward.

##### optional flags for `package publish`
...
- `--apply`
  ...
  - Perform server writes. Without it, the command validates and builds only.

- `--smoke`

  - Apply the install Bundle in the calling project to prove it is well-formed, then delete what it created

- `--force`

  - Overwrite an already-published version instead of refusing
  - Recorded on the publish `AuditEvent`

- `--catalog-project <projectId>`
  - Project to publish the `Package`, `PackageRelease`, and `Binary` into
