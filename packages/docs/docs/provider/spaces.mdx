...
The Provider app source lives in [`examples/medplum-provider`](https://github.com/medplum/medplum/tree/main/examples/medplum-provider). The three bots behind the chat input are installed by the Medplum team. Use this page to set Spaces up in your own project, understand what it does behind the chat input, and tune the parts that are designed to be tuned.

:::caution[Medplum Team Setup Required]
Spaces requires setup by the Medplum team: the `ai` and `bots` project features, plus the three Spaces bots themselves. [Contact us](mailto:info+spaces@medplum.com?subject=Spaces%20for%20Medplum) to enable Spaces for your project.
:::
...
:::caution
If the key is missing, the `$ai` operation returns `OpenAI API key not configured in project secrets` and every Spaces turn fails. Set this before rolling Spaces out to users.
:::

### Request The Spaces Bots

Spaces depends on three bots that the Medplum team installs in your project. [Contact us](mailto:info+spaces@medplum.com?subject=Spaces%20for%20Medplum) to have them provisioned.

The Provider UI resolves each bot by `Identifier` (system `https://www.medplum.com/bots`), so the identifier values below are load-bearing. They are worth knowing: the same values name the system-prompt Communications you author in the next section, and they are the first thing to check when a Spaces turn silently fails.

| Identifier Value             | Role                                                                |
| ---------------------------- | ------------------------------------------------------------------- |
| `ai-fhir-request-tools`      | Translator. Emits `fhir_request` tool calls and a `visualize` flag.  |
| `ai-resource-summary-sse`    | Streaming summary bot (Server-Sent Events).                          |
| `ai-component-generator-sse` | Streaming Recharts / Mantine `Chart()` JSX generation.               |

Once the bots are installed, you can confirm the wiring in the [Medplum App](https://app.medplum.com): each [`Bot`](/docs/api/fhir/medplum/bot) resource should carry an `identifier` entry whose `system` is `https://www.medplum.com/bots` and whose `value` matches the table above. A missing or mistyped value silently breaks the feature.
...
### Default Prompts And Overrides

If Spaces has been set up in your project, default base prompts are already in place for all three bots, so you don't need to author all three Communication resources yourself before Spaces works.

Because of this, a search for a prompt Communication by identifier can return two matches: the default base prompt, and, once you've created one, your own override with the same identifier. The bots resolve this with a preference rule rather than assuming a single match: they search by identifier, then prefer whichever result's `meta.project` equals the current project, falling back to the most recently updated match if no override exists.

In practice:

- With no override, the default base prompt is used automatically.
- Once you create a Communication with the same identifier, it takes over immediately for your users – you don't need to edit or delete the default one. 
...
Spaces' behavior – when it calls FHIR, how it phrases summaries, which chart types it reaches for – is determined almost entirely by the three system-prompt Communications, not by the bot code (the bots are thin wrappers around `$ai`). The next user message picks up a changed prompt immediately, without a bot redeploy.

Do not edit the default base prompts in place (see [Default Prompts And Overrides](#default-prompts-and-overrides) above). Instead, create your own Communication with the same identifier. The bots prefer whichever match's `meta.project` equals the current project, so your copy takes over automatically, with no redeploy needed.
