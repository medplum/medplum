...
Questionnaires are used to organize a collection of questions to gather information from various participants in the healthcare process.

These are modeled in FHIR with the [`Questionnaire`](/docs/api/fhir/resources/questionnaire) resource, while the responses to these questions are modeled with the [`QuestionnaireResponse`](/docs/api/fhir/resources/questionnaireresponse) resource.

## Creating a Questionnaire

A [`Questionnaire`](/docs/api/fhir/resources/questionnaire) represents the questions, rules for answering the questions, and meta data used to define what it should be used for. It allows you to create complex forms with nested and conditional questions.
...
| `item`        | An object containing the questions and groups of questions. Also includes any rules defined for answering. |                                                                                       | [See below](#the-questions-on-a-questionnaire)                         |
...
| `status`      | Displays whether the `Questionnaire` is active, in a draft, etc.                                           | [Publication Status Codes](https://www.hl7.org/fhir/valueset-publication-status.html) | draft                                                                  |

### The Questions on a Questionnaire

The actual questions on a [`Questionnaire`](/docs/api/fhir/resources/questionnaire) are defined on the `item` element. This element allows you to structure your questions both as individual questions and in groups. You can also provide additional context or instructions on how the questions should be answered.

One of the most important properties on the `item` is `linkId`, which is used to map the questions to the corresponding answers on a [`QuestionnaireResponse`](/docs/api/fhir/resources/questionnaireresponse) resource. These must all be unique within the [`Questionnaire`](/docs/api/fhir/resources/questionnaire) to ensure that they can be accurately tracked. The order of the questions cannot guarantee accuracy as not all questions are always required to be answered, so there may not be the same number of questions and answers.

| **Property**     | **Description**                                                                                                                         | **Example**                                |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| `linkId`         | A unique identifier within the `Questionnaire` that maps directly to the answer for the equivalent `item` on a `QuestionnaireResponse`. | 2.1                                        |
...
| `initial`        | A value that will pre-populate the field when the question renders.                                                                     | 1970-01-01                                 |
...
- Conditionally displaying certain questions

#### Initial Values

To set an initial value for your question, you can use the `item.initial` field. This can be set to any value type, but should be the same as the `type` field of the item it is on. The `initial` field can be overwritten by the user responding to the [`Questionnaire`](/docs/api/fhir/resources/questionnaire), but the value will persist if the user does not change it.

In addition to `initial`, there is also the `initialSelected` field which works in the same way, but applies to questions that have an `answerOption` field, and is a property on that field (i.e. `answerOption.initialSelected`). In this case you set an initial choice that is automatically selected when a question renders.
...
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ----------- |
| `question`   | The `linkId` for the `item` whose answer determines if this `item` will appear.                                                                                |                                                                                                     | 1.1         |
...
| `subject`       | Who/what the answers from the response apply to, but not necessarily who actually answered the questions.         |                 | Patient/homer-simpson                                   |
| `item`          | The responses to the questions.                                                                                   |                 | [See below](#the-responses-to-questions)                |
| `author`        | The individual who received and recoreded the responses, but not necessarily who actually answered the questions. |                 | Practitioner/receptionist                               |
| `authored`      | The date that the answers were gathered.                                                                          | dateTime        | 2023-11-18                                              |
| `encounter`     | A reference to the `Encounter` that the response is a part of.                                                    |                 | Encounter/homer-simpson-physical                        |

### The Responses to Questions

The answers on a [`QuestionnaireResponse`](/docs/api/fhir/resources/questionnaireresponse) are stored on the `item` element. It is very similar to the `item` element on a [`Questionnaire`](/docs/api/fhir/resources/questionnaire), allowing you to model the responses so that they match the structure of the [`Questionnaire`](/docs/api/fhir/resources/questionnaire) they are answering.
...
| `linkId`          | The item from the corresponding `Questionnaire` that this answer responds to.                               | 2.1                  |

<details>
  <summary>Example: A `QuestionnaireResponse`</summary>
