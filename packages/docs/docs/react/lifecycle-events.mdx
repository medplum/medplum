...
When wrapping your app in a `MedplumProvider`, you should be aware of the [lifecycle events](https://todo.real) of `MedplumClient`. Each of the lifecycle events will trigger a rerender at the root of your; 
...
function App() {
  ...
  if (!profile) {
    // Redirect users back to signin whenever there is no profile and anytime after initial load
    navigate("/signin");
    return null;
  }
