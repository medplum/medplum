...
useSubscription(
  'Communication',
  (bundle: Bundle) => {
    // Subscription bundles have a specific structure:
    // - entry[0] contains SubscriptionStatus (metadata about the event)
    // - entry[1] contains the actual resource that triggered the subscription
