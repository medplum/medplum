...
This example will walk through how to implement this pattern using FHIR objects and [FHIR Subscriptions](https://www.hl7.org/fhir/subscription.html). For those unfamiliar with FHIR Subscriptions, it is helpful to think of them as a kind of webhook that is triggered on changes to a FHIR search filter.
...
- Update the ServiceRequest as it moves through a workflow. For example, the sample is collected, the sample is analyzed, a diagnostic report is created, etc.
...
This section shows how to create a ServiceRequest for a lab test that belongs to a Patient using the API. Notably, the snippet below _conditionally creates_ a Patient if one does not exist, and creates a service request for a lab panel.

<MedplumCodeBlock language="ts" selectBlocks="core-imports,create-service-request">
{ExampleCode}
</MedplumCodeBlock>

Using this code snippet, a ServiceRequest is created and linked to a Patient. You should be able to see the [Patient](https://app.medplum.com/Patient) created here and the [ServiceRequest](https://app.medplum.com/ServiceRequest) created here.
...
After the ServiceRequest is created, it needs to be updated continuously as it moves through a workflow. It can be hard to visualize what is happening here, but the way to think about this is from the perspective of a Patient getting a lab test.
