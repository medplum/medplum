...
<details>
  <summary>Example: 10 - 100 mg/dL</summary>
...
<details>
  <summary>Example: Less than or equal to 5 mg/dL </summary>
...
<details>
  <summary>Example: Greater than or equal to 20 mg/dL </summary>
  <MedplumCodeBlock language="ts" selectBlocks="highRange">
    {ExampleCode}
  </MedplumCodeBlock>
</details>

In practice, an [`ObservationDefinition`](/docs/api/fhir/resources/observationdefinition) will define multiple reference ranges for a given patient population, to provide interpretations for each value of the [`Observation`](/docs/api/fhir/resources/observation).

<details>
  <summary>Example: Multiple Ranges</summary>
...
<details>
  <summary>Example: Age and gender dependent reference ranges</summary>
...
| `critical`  | Defines the **critical** values for the observation, also known as "panic" values. Observing values in these ranges often requires special handling to notify patients. |
...
The `validCodedValueSet`, `normalCodedValueSet`, `abnormalCodedValueSet`, and `criticalCodedValueSet` fields mirror the functionality of `absolute`, `reference`, and `critical` reference ranges, and are used to define the interpretation of a qualitative observation.
...
| ----------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `validCodedValueSet`    | `absolute`  | Enumerates _all_ valid qualitative values for this Observation type.                                                                       |
...
| `abnormalCodedValueSet` | `reference` | Enumerates all codes that signify an _abnormal_ result for this Observation. It should be a subset of the `validCodedValueSet`.            |
