...
Our recommendations are informed by the [Order Catalog Implementation Guide](http://hl7.org/fhir/uv/order-catalog/2020Sep/), which was developed with contributions from Labcorp, Quest Diagnostics, and other industry leaders.
...
The first step in building your catalog is to define which clinical quantities, or "observations", you will measure. In a lab context, an example of a clinical observation would be an HBA1c percentage. For a given patient, this observation might have a value such as 5.3%.
...
| `action.code`                | The code for the corresponding lab procedure                                                                                                                          | [LOINC](./loinc)                                                                                               | [55231-5](https://loinc.org/55231-5) - Electrolytes panel - Blood                |
| `action.definitionCanonical` | The "canonical url" of the [`ActivityDefinition`](/docs/api/fhir/resources/activitydefinition) representing the procedure (see [below](#activitydefinitions)) |                                                                                                                | http://example.org/ActivityDefinition/electrolyte-panel                          |
...
There is a bit of an art to determining how to divide the individual tests into procedures, and it requires an understanding of your lab operations. Some considerations to help guide you:
...
<details>
  ...
  <p>
    This PlanDefinition reuses the Electrolyte Panel from the previous example, but adds a free testosterone test.
  </p>
    <h5>
...
These advanced scenarios are out of scope for this guide, but you can check out [this implementation guide](http://hl7.org/fhir/uv/order-catalog/2020Sep/exlabservices.html) for examples of how these might be implemented

:::

## Querying your catalog

You can query all [`PlanDefinitions`](/docs/api/fhir/resources/plandefinition) that represent a laboratory procedure and the associated [`ActivityDefinitions`](/docs/api/fhir/resources/activitydefinition) with this query:
...
1. Query all ActivityDefinitions for a given PlanDefinition using the [`_include`](/docs/search/includes#_include-and-_revinclude) directive
