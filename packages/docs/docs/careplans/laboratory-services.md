...
Key amongst these is the `qualifiedInterval` element, which is used to define *how results should be interpreted.* See our [guide on reference ranges](./reference-ranges) for more information.
...
In the next section, we'll learn more about the `PlanDefinition.action` element, which defines the lab procedures used to *fulfill* the diagnostic service order.
