...
- Licensing: model a practitioner's role per `HealthcareService` or service line, with the licensed jurisdictions in `PractitionerRole.location` (multiple states may share one role). One role per state is not a scheduling requirement. Scheduling operations do not enforce licensure; validate eligibility in the application's controlled booking path, and enforce data access with `AccessPolicy`.  [scheduling/state-by-state-licensure]
