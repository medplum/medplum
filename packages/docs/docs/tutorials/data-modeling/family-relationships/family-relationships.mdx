...
The FHIR spec provides a number of Resources for modeling a patient's relationship to other individuals, but how you use these tools will depend on your specific clinical setting and workflow. Having your data well modeled in FHIR is standards compliant and can help you compute compliance metrics such as [CMS queries](https://www.medplum.com/docs/compliance/onc#materials-and-usage).
...
|        [**RelatedPerson** ➡ **Patient**](#approach-2)        |                                        Yes                                         |                            Rarely                            |                    Individual Patient                    | <ul><li>Modeling Insurance coverage where the _beneficiary_ and _subscriber_ are different people </li><li>Parents acting on behalf of minors</li></ul> |
...
```graphql
{
  # Read the Patient with specific ID
  Patient(id: "patient-id") {
    ...
    # Read the Patient.contact field to get a list of family member contacts
    contact {
      relationship {
        text
      }
...
<img alt="Approach #2" src={approach2Diagram} style={{ maxHeight: '400px', width: 'auto' }} />
...
```graphql
{
  ...
  Patient(id: "patient-id") {
    ...
    # Get a list of all RelatedPerson resources whose 'patient'
    # field points to the above patient (id = 'patient-id)
...
```graphql
{
  ...
  Patient(id: "patient-id") {
    ...
    RelatedPersonList(_reference: patient) {
      ...
      # Get a list of all the Patients whose "link.other"
      # field points to the above RelatedPerson
      # (Creates a nested list inside each RelatedPerson entry)
      PatientList(_reference: link) {
        id
        name {
          given
          family
...
Another common use case for this approach is in neonatal care. Many common neonatal workflows, such as newborn hearing tests, postpartum-depression screening, and breastfeeding metrics all require modelign both the mother and child as patients to track their indivdual encounters, medications, labs, and imaging.
...
<img alt="Approach #4" src={approach4Diagram} style={{ maxHeight: '400px', width: 'auto' }} />
</TabItem>
<TabItem value="data" label="JSON" >
...
```graphql
{
  # Read the Group with specific ID
  Group(id: "group-id") {
    id
    name

    # Read the list of group members, where each entity
    ...
    member {
      entity {
        ...
        # Medplum's graphQL implementation allows you to read the resource
        # referenced by 'entity' using the 'resource' field
        resource {
          # The '... on Patient' syntax is needed to tell the graphQL engine
          # that this field references a Patient resource
          ... on Patient {
            id
            name {
              given
