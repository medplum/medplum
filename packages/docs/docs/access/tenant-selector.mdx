...
flowchart TB
    ...
    subgraph API["API Layer (ProjectMembership + AccessPolicy)"]
    ...
    subgraph App["Application Layer"]
        AT["Active Tenant = Org B"]
        Q["Patient?_compartment=Organization/org-b"]
        AT --> Q
    end
...
_`ProjectMembership` controls which tenants a user may access (allowed tenants). The application separately tracks which tenant is currently selected (active tenant)._
...
flowchart TB
    ...
    subgraph L1["Login 1 — Org A"]
        PM1["ProjectMembership\nOrg A only"]
    end

    subgraph L2["Login 2 — Org B"]
        PM2["ProjectMembership\nOrg B only"]
    end

    User --> L1
    User --> L2
    PM1 --> OA[Org A Patients]
    PM2 --> OB[Org B Patients]
...
flowchart TB
    User([Practitioner])
    PM["Single ProjectMembership\n(Org A + Org B allowed)"]
    User --> PM
    PM --> OA[Org A Patients]
    PM --> OB[Org B Patients]

    subgraph App["Application UI"]
        AT["Active Tenant\n(stored in UserConfiguration)"]
        QA["?_compartment=Organization/org-a"]
        QB["?_compartment=Organization/org-b"]
        AT -->|"Org A selected"| QA
        AT -->|"Org B selected"| QB
    end
