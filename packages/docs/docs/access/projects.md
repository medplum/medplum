...
1. If `defaultAccessPolicy` contains a row whose `resourceType` matches the membership profile type (`Patient`, `Practitioner`, or `RelatedPerson`), and that row has a non-empty `access` list, those [`ParameterizedAccess`](/docs/api/fhir/medplum/projectmembership) entries are copied onto the membership (cloned so later edits to the project do not mutate existing memberships).
