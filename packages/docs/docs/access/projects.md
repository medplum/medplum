...
| `allowedPractitionerEmailDomain` | Email domains allowed for new Practitioner accounts. When present, an [invite](/docs/api/project-admin/invite) for a Practitioner whose email address is outside of these domains is rejected. Domains are matched exactly and case-insensitively, so `example.com` does not allow `mail.example.com`. This does not apply to Patient or RelatedPerson invites, or to invites that supply an `externalId` instead of an email address.                                                                                                                                                                                                 |         |
