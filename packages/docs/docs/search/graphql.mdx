...
GraphQL queries allow you to request specific fields on resources. In a FHIR GraphQL query, you will use the resource type as the root, followed by the ID in parentheses. The requested fields are enclosed in curly braces.
...
FHIR GraphQL supports searching for multiple resources using the `<ResourceType>List`. To perform a search, you need to specify the resource type and the search parameters, similarly to [basic REST search](/docs/search/basic-search).

However, in GraphQL, the **search parameter names use snake_case** instead of the kebab-case commonly used in the FHIR REST API. GraphQL also allows you to [alias returned fields](https://devinschulz.com/rename-fields-by-using-aliases-in-graphql/) to make the results more readable.

For example, to search for a list of `Patient` resources with a specific name and city:
...
FHIR resources often reference other resources. The FHIR GraphQL API contains a special `resource` element to resolve these references and retrieve the nested resources.
...
The FHIR GraphQL syntax is a powerful way to query for multiple related resources in a single HTTP call, and the following example combines all the concepts we've covered so far.

The query below searches for a list of `Patients` named `"Eve"`, living in `"Philadelphia"`, and then searches for all `DiagnosticReports` linked to each `Patient` along with their corresponding `Observations`.
...
With a deeper understanding of the FHIR GraphQL syntax, you can now leverage the power of the GraphQL API to build efficient and flexible FHIR queries. Remember to experiment with the API at [graphiql.medplum.com](https://graphiql.medplum.com/) as you develop your application.
