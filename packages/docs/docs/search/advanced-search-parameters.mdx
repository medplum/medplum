...
:::warning `_total=accurate` limitation

The `_total=accurate` search parameter will only accurately return if the total is under 1 million.
...
:::warning Mixing `_deleted` with other parameters
