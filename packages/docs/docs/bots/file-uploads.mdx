...
To upload to a file using http, you will have to submit an HTTP request with the `Content-Type` `multipart/form-data`. Modern JavaScript runtimes provide the standard [`FormData`](https://developer.mozilla.org/en-US/docs/Web/API/FormData) API to make it easier to create form data.
...
Next, we'll construct the request body using `FormData`.
...
<MedplumCodeBlock
  language="ts"
  selectBlocks="coreImports,botFn,createPdf,checkBinary,downloadPdf,nl,formData,nl,postForm,closeFn"
>
  {ExampleCode}
</MedplumCodeBlock>
