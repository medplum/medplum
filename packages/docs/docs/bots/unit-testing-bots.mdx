...
4. Query mock resources and assert your test criteria

The [finalize-report bot tests](https://github.com/medplum/medplum-demo-bots/blob/main/src/finalize-reports.test.ts) are a great example of this pattern.
...
After running the Bot, the [finalize-report bot's tests](https://github.com/medplum/medplum-demo-bots/blob/main/src/finalize-reports.test.ts) read the updated `DiagnosticReport` and `Observation` resources to confirm their status.
