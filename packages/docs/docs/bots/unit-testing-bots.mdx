...
- Token search modifiers `:above` and `:below` (require a real terminology service)
- Reverse-chain `_has` search
- Schema validation is **opt-in** via `new MemoryRepository({ validateResources: true })` to preserve backwards compatibility with existing test fixtures.
- **Authentication**
