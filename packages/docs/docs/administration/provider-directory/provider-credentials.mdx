...
The `issuer` is typically the state (e.g., "State of New York"). Although this element is typically a reference to an `Organization`, for simplicity we recommend using the `display` element.

:::note Da Vinci PDEX Extension
For systems requiring interoperability with payer networks, the [Da Vinci PDEX `whereValid` extension](https://build.fhir.org/ig/HL7/davinci-pdex-plan-net/StructureDefinition-qualification.html) allows encoding license jurisdictions on qualifications. However, this cannot be queried without a custom SearchParameter. For most use cases, the Location + PractitionerRole pattern below is recommended.
:::

## Finding Practitioners by State

While `Practitioner.qualification` stores license details, it doesn't support querying
practitioners by state out-of-the-box. For operational use cases like assigning care
based on a patient's location, use `Location` + `PractitionerRole`.

### Setting Up State Locations

Create a `Location` resource for each state (one-time setup):

<MedplumCodeBlock language="ts" selectBlocks="state-location">
  {ExampleCode}
</MedplumCodeBlock>

### Linking Practitioners to States

Create a `PractitionerRole` for each state where a practitioner is licensed:

<MedplumCodeBlock language="ts" selectBlocks="practitioner-role-state">
  {ExampleCode}
</MedplumCodeBlock>

### Querying Licensed Practitioners

Find all practitioners licensed in a specific state:

<MedplumCodeBlock language="ts" selectBlocks="query-by-state">
  {ExampleCode}
</MedplumCodeBlock>

:::tip
Use `Practitioner.qualification` to store license type (MD, RN, etc.) and issuer. Use `PractitionerRole` + `Location` for queryable state assignments and care routing. See [Including Linked Resources](/docs/search/includes) for advanced queries.
:::
