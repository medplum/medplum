...
A core data object in FHIR is called a [**Resource**](https://www.hl7.org/fhir/resource.html). You can think of Resources as **objects** in object oriented languages. The FHIR standard defines a set of [**Resource Types**](/docs/api/fhir/resources), similar to **classes**, that have been built for healthcare applications.

Resources can represent a broad range of healthcare ideas, from very **concrete healthcare items** (e.g. [Patient](/docs/api/fhir/resources/patient), [Medication](/docs/api/fhir/resources/medication), [Device](/docs/api/fhir/resources/device)) or more **abstract concepts** (e.g. [Procedure](/docs/api/fhir/resources/procedure), [Care Plan](/docs/api/fhir/resources/careplan), [Encounter](/docs/api/fhir/resources/encounter)).
...
The example below shows an example **[Patient](/docs/api/fhir/resources/patient) resource**, represented as JSON. Here we can see that the `Patient` contains multiple `Elements`, including: `name`, `telecom`, and `address`.
...
The example below shows a [MedicationRequest](/docs/api/fhir/resources/medicationrequest) resource two references: **`subject` (i.e. the patient)** and **`requester` (i.e. physician)**.
...
You can find the search parameters on the [reference page](/docs/api/fhir/resources) for each resource. **Refer to the [FHIR Search](https://www.hl7.org/fhir/search.html) documentation** for how construct search queries using these parameters.

### Example

For example the [Medication resource](/docs/api/fhir/resources/medication#search-parameters) defines the following search parameters:
...
In Medplum, a powerful feature is to to **use a [Medplum Bot](/docs/tutorials/bots)** as the endpoint of the `rest-hook` channel. This allows you to run an arbitrary piece of code in response to data changes and automate your medical workflows. See our [Bot-Subscription tutorial](/docs/tutorials/bots/bot-for-questionnaire-response) for more information.
