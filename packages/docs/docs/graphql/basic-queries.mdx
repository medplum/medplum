...
When using GraphQL, [access policies](/docs/access/access-policies) are enforced, so users will not be able to read or edit any resources they do not have access to.
:::
