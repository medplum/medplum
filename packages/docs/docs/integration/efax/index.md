...
For the simplest setup, if you only need one fax number, we recommend `/fhir/R4/$receive-efax`.

| Endpoint | Use Case | User ID Source |
|----------|----------|----------------|
| `POST /fhir/R4/$receive-efax` | Clinic-wide shared fax | Project secret `eFaxUserId` |
