...
keywords: [Questionnaire, QuestionnaireResponse, linkId, repeats, group, SDC, extract, bot, upsert]
---
...
**Group linkIds as resource-type identifiers:** The group-level linkId (`allergies`, `medications`, `medical-history`, `vaccination-history`, `coverage-information`) identifies the response items that are mapped to a resource template. In SDC extraction, placing `templateExtract` on the group causes the template to be evaluated once for each repeated response item.

For general Questionnaire mechanics (item types, enableWhen, nested items), see [Questionnaires & Assessments](/docs/questionnaires).
...
For full details on SDC annotation syntax, template resources, and FHIRPath rules, see [Parsing Questionnaire Responses](/docs/questionnaires/parsing-questionnaire-responses).
...
Bots can traverse `QuestionnaireResponse.item` and `answer` directly, or use application-specific parsing helpers. This is useful when the transformation needs branching, external lookups, or side effects that cannot be represented cleanly as SDC FHIRPath mappings.
...
For a complete SDC implementation, see the [Patient Intake Questionnaire](https://github.com/medplum/medplum/blob/main/examples/medplum-patient-intake-demo/data/core/patient-intake-questionnaire-full-sdc.json) in the Patient Intake Demo. It keeps the extraction templates and FHIRPath mappings alongside the form.
...
For consent auditing, digital signature capture, and third-party e-signature integration, see [Consent and Signatures](/docs/consent).
...
- [Parsing Questionnaire Responses](/docs/questionnaires/parsing-questionnaire-responses) — SDC annotation guide and `$extract` API details
