...
Flag to enable server-scoped rest-hook [Subscriptions](/docs/subscriptions/server-scoped-subscriptions). When enabled, the subscription worker evaluates not only the subscriptions within a resource's own project, but also "server-scoped" subscriptions that are not scoped to any project (i.e. stored in the system project). This allows a single set of subscriptions to fire for changes across every project on the server, which is useful for large self-hosted deployments that use a project-per-tenant model but need a central data-processing flow for all tenants.

Server-scoped subscriptions can only be created by a super admin (they are the only ones who can create project-less resources), and `AuditEvent` resources generated by these firings are themselves project-less and live in the system scope. See [Server-Scoped Subscriptions](/docs/subscriptions/server-scoped-subscriptions) for full details.
