...
| `defaultAccessPolicy`        | Per–profile-type defaults for new [`ProjectMembership`](/docs/api/fhir/medplum/projectmembership) records that omit `access` and `accessPolicy`. Each element has `resourceType` (for example `Patient`) and `access` ([`ParameterizedAccess`](/docs/api/fhir/medplum/projectmembership) list). See [Default access policies](/docs/access/projects#default-access-policies) for precedence with `defaultPatientAccessPolicy`. |         |
| `defaultPatientAccessPolicy` | **Deprecated.** Legacy single [`AccessPolicy`](/docs/access/access-policies) for new **Patient** memberships when no matching `defaultAccessPolicy` row supplies parameterized access. Still supported; prefer `defaultAccessPolicy` for new integrations. Open registration still requires *some* default patient access—see [open patient registration](/docs/user-management/open-patient-registration). |         |
