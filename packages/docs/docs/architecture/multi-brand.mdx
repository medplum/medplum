...
Brand applications live in the `apps/` workspace, separate from shared packages:
...
The root `package.json` defines three workspaces:

```json
{
  "workspaces": [
    "packages/*",   // Shared libraries (@mrd/ui, @mrd/agents)
    "examples/*",   // Medplum demo applications
    "apps/*"        // Production brand applications
  ]
}
```
...
1. **Copy the template** from an existing brand in `apps/` folder
2. **Update package.json** with new brand name (`@mrd/newbrand`)
3. **Create brand theme** in `packages/ui/src/themes/newbrand.css`
