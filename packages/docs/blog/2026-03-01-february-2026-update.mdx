...
- **`_count` parameter for [`$find`](/docs/scheduling/appointment-find)** — Limits the number of available slots returned, improving efficiency for high-volume scheduling queries
...
- **Allergy representation** — Clarified allergy status and AllergyIntolerance formatting in the [chart data model](/docs/charting/chart-data-model#allergies-and-intolerances), contributed by [Aaron Hong](https://github.com/aaronhong)
