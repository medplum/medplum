...
import { useEvent, usePluralForm, useSearchQueryString } from '@docusaurus/theme-common';
...
export function AlgoliaSearch(): ReactNode {
  const {
    i18n: { currentLocale },
        ...
        <main>
          {searchResultState.items.map(({ title, url, summary, breadcrumbs }, i) => (
            <article key={i} className={styles.searchResultItem}>
                ...
                <nav aria-label="breadcrumbs">
                  <ul className={clsx('breadcrumbs', styles.searchResultItemPath)}>
                    {breadcrumbs.map((html, index) => (
                      <li key={index} className="breadcrumbs__item" dangerouslySetInnerHTML={{ __html: html }} />
                    ))}
                  </ul>
                </nav>
              )}
              ...
              {summary && (
                <p className={styles.searchResultItemSummary} dangerouslySetInnerHTML={{ __html: summary }} />
              )}
            </article>
          ))}
        </main>
