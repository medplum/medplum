...
function LegendSwatch({ kind, dashed = false, label }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9 }}>
      ...
      />
      <span style={{ fontSize: 12, color: C.text, lineHeight: 1.3 }}>{label}</span>
    </div>
  );
}
...
function MedplumDiagram({ active = null, peek = null, onSelect }) {
  ...
  /* Solid-fill context (the purple hosted-platform pill) dims via a filter wash instead
     of opacity, so its white-on-purple fill never goes translucent. */
  const ctxClassSolid = `${styles.dimmable} ${active ? styles.dimmedSolid : ''}`;
  /* SVG overlays (connectors) can't take the class treatment per-line; dim wholesale
     so the wiring recedes with the rest of the figure when a region is spotlit. */
  const svgDim = { opacity: active ? 0.4 : 1, transition: 'opacity 200ms ease' };
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto', display: 'flex', gap: 28, alignItems: 'flex-start' }}>
        <div ref={rootRef} style={{ flex: 1, minWidth: 0, position: 'relative' }}>
          ...
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', columnGap: 14, rowGap: 0 }}>
              ...
              {/* Row 3 — TypeScript / JavaScript SDK (foundation, clickable) */}
              <Card
          ...
          >
            ...
            <div style={{ position: 'relative' }} ref={refs.platformBox}>
              <div
                className={`${styles.hostedPill} ${ctxClassSolid}`}
                style={{
                  position: 'absolute',
                  top: -12,
                  right: 22,
                ...
                <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
                  ...
                  >
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      />
                      <Card nodeRef={refs.bot} {...clk('Bots')} kind="extensible" icon="robot" name="Bots" />
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ fontSize: 11.5, color: C.sub, whiteSpace: 'nowrap' }}>
                        SNOMED · ICD-10 · LOINC
                      </span>
                    </div>
                  </div>
                ...
                return (
                  <svg
                    style={{
                      ...
                      pointerEvents: 'none',
                      overflow: 'visible',
                      opacity: active && active !== 'Medplum Auth' ? 0.4 : 1,
        ...
        {/* ═══════════════════ LEGEND (vertical, side column) ═══════════════════ */}
        <div style={{ flexShrink: 0, width: 188, paddingTop: 4 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: C.text, marginBottom: 10 }}>Legend</div>
          <div
            style={{
