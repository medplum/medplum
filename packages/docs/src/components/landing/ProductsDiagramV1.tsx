  ...
  return (
    <div ref={nodeRef} className={`${shadowClass} ${className}`} {...rest} style={{
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        <div style={{ ...LABEL_TEXT, whiteSpace: 'pre-line' }}>
          {name}
        </div>
        {sub &&
        <div style={{ ...SUB_TEXT, marginTop: 2 }}>{sub}</div>
        }
      </div>
    ...
    <div className={styles.root} style={{
      ...
      <div ref={rootRef} style={{ position: 'relative', paddingTop: '1.5rem' }}>
        ...
        <svg style={{
          ...
          {R.subs && R.bot && R.intBox && R.intFirst && R.intThird && R.platformBox && R.bridge && (() => {
        ...
        <div ref={refs.appsRow} style={{ position: 'relative', zIndex: 2, marginBottom: 56,
          ...
          <div ref={refs.medplumAppsBox} style={SECTION_WRAP}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--dg-inner-gap)' }}>
              <Card nodeRef={refs.medplumProv} kind="extensible" icon="stethoscope"
                name="Medplum Provider" sub="Example EHR" extensible compact />
              <Card nodeRef={refs.medplumApp} kind="extensible" icon="terminal"
                name="Medplum App" sub="Admin / Dev Console" extensible compact />
            </div>
        ...
        <div style={{ position: 'relative', zIndex: 2, display: 'grid',
          gridTemplateColumns: '1fr 240px', gap: 56, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              {GOVERNANCE_SIDEBAR.flatMap((item, index) => {
                const nodes = [
                  <div key={item.label} style={GOVERNANCE_CELL}>
                    ...
                    <div style={{ ...LABEL_TEXT, flex: 1, minWidth: 0 }}>
                      {item.label}
                    </div>
                    <ExtensibleMark />
                  </div>,
            ...
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 'var(--dg-inner-gap)' }}>

            {/* REST API + Auth API — one rounded block, split internally */}
            <div style={platformBlock({ display: 'flex', alignItems: 'stretch' })}>
              <div ref={refs.fhirApi} style={{
                ...PLATFORM_ROW,
                flex: 1,
                minWidth: 0,
              }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--dg-inner-gap)', marginLeft: 16, marginRight: 16 }}>
              <Card nodeRef={refs.subs} kind="extensible" icon="bell" name={'Webhook / WebSocket\nSubscriptions'} extensible compact />
              <Card nodeRef={refs.bot} kind="extensible" icon="robot" name="Bots" extensible compact />
            </div>

            {/* FHIR Datastore — freestanding, fully rounded */}
            <div ref={refs.datastore} style={platformBlock({
              ...PLATFORM_ROW,
              padding: '8px',
            })}>
            ...
            <div style={{ fontSize: 'var(--dg-size-card)', lineHeight: 'var(--dg-leading-sub)', color: C.muted, padding: '4px 6px' }}>
          ...
          <div style={{
            ...
            <Card nodeRef={refs.byoIdp} kind="customer" icon="userKey"
              name="External Identity Provider" sub="Optional" compact />
            ...
            <Card nodeRef={refs.bridge} kind="extensible" icon="arrowsUpDown" iconRotate={90}
              name="Medplum Bridge" sub="On-prem · HL7 / DICOM" extensible compact />

            {/* Integrations (gray section box). */}
            <div ref={refs.intBox} style={SECTION_WRAP}>
              <div style={SECTION_LABEL}>Integrations</div>
              <Card nodeRef={refs.intFirst} kind="extensible" icon="apps" iconRotate={-90} name="First-party" extensible compact />
