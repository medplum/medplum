...
PID|1||11111^^^MRN^MR||JONES^BOB^C||19700101|M`;
  ...
  describe('Deferred commit ACK', () => {
    const testMessage =
      Hl7Message.parse(`MSH|^~\\&|SENDING_APP|SENDING_FAC|REC_APP|REC_FAC|20240218153044||DFT^P03|MSG_DEF_01|P|2.3
PID|1||12345^^^MRN^MR||DOE^JOHN^A`);

    function getAckCode(writeBuffer: Buffer): string | undefined {
      const decoded = iconv.decode(writeBuffer.subarray(1, writeBuffer.byteLength - 2), 'utf-8');
      return Hl7Message.parse(decoded).getSegment('MSA')?.getField(1)?.toString();
    }

    function getAckMsaText(writeBuffer: Buffer): string | undefined {
      const decoded = iconv.decode(writeBuffer.subarray(1, writeBuffer.byteLength - 2), 'utf-8');
      return Hl7Message.parse(decoded).getSegment('MSA')?.getField(3)?.toString();
    }
    ...
    test('standard mode: auto-CA is suppressed when deferred', async () => {
      ...
      expect(connection.getDeferredCommitAck()).toBe(true);

      await connection.close();
    });

    test('aaMode: auto-AA is suppressed when deferred', async () => {
      const mockSocket = new MockSocket();
      const connection = new Hl7Connection(mockSocket as any, undefined, 'aaMode');
      connection.setDeferredCommitAck(true);
    ...
    test('ackCommit sends AA in aaMode', async () => {
      const mockSocket = new MockSocket();
      const connection = new Hl7Connection(mockSocket as any, undefined, 'aaMode');
      connection.setDeferredCommitAck(true);
      ...
      expect(mockSocket.write).toHaveBeenCalledTimes(1);
      expect(getAckCode(mockSocket.write.mock.calls[0][0] as Buffer)).toBe('AA');

      await connection.close();
    });

    test('ackCommit is no-op when not in enhanced mode', async () => {
      const mockSocket = new MockSocket();
      const connection = new Hl7Connection(mockSocket as any, undefined, undefined);
      connection.setDeferredCommitAck(true);
    ...
    test('nackCommit sends CR in standard mode with optional reason in MSA.3', async () => {
      ...
      connection.nackCommit(testMessage, 'CR', 'storage error');

      expect(mockSocket.write).toHaveBeenCalledTimes(1);
      const writeBuffer = mockSocket.write.mock.calls[0][0] as Buffer;
      expect(getAckCode(writeBuffer)).toBe('CR');
    ...
    test('nackCommit sends AR in aaMode', async () => {
      const mockSocket = new MockSocket();
      const connection = new Hl7Connection(mockSocket as any, undefined, 'aaMode');
      connection.setDeferredCommitAck(true);

      connection.nackCommit(testMessage, 'AR');

      expect(mockSocket.write).toHaveBeenCalledTimes(1);
      expect(getAckCode(mockSocket.write.mock.calls[0][0] as Buffer)).toBe('AR');
    ...
    test('toggling deferred off restores auto-ACK behavior', async () => {
      const mockSocket = new MockSocket();
      const connection = new Hl7Connection(mockSocket as any, undefined, 'standard');
      connection.setDeferredCommitAck(true);
      ...
      connection.setDeferredCommitAck(false);
      connection.dispatchEvent(new Hl7MessageEvent(connection, testMessage));

      // Auto-CA fires once for the second dispatch.
      expect(mockSocket.write).toHaveBeenCalledTimes(1);
      expect(getAckCode(mockSocket.write.mock.calls[0][0] as Buffer)).toBe('CA');
