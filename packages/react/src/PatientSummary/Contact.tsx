import { Box, Group, Text, Collapse, ActionIcon, UnstyledButton, Tooltip } from '@mantine/core';
import { Patient, Resource, ContactPoint } from '@medplum/fhirtypes';
...
export function Contact(props: ContactProps): JSX.Element {
  ...
  const [copied, setCopied] = useState<{ address: boolean; phone: number | null; email: number | null }>({ address: false, phone: null, email: null });
  ...
  let addressString = '';
  if (address) {
    if (address.line?.[0]) { addressString += address.line[0]; }
    if (address.line?.[1]) { addressString += (addressString ? '\n' : '') + address.line[1]; }
    if (address.city || address.state || address.postalCode) {
      addressString += (addressString ? '\n' : '');
      addressString += address.city || '';
      if (address.state) { addressString += (address.city ? ', ' : '') + address.state; }
      if (address.postalCode) { addressString += (address.city || address.state ? ' ' : '') + address.postalCode; }
    }
  }
  ...
  function handleCopyWithFeedback(e: React.MouseEvent, value: string, type: 'address' | 'phone' | 'email', index?: number): void {
    handleCopy(e, value);
    if (type === 'address') {
    ...
    <Box style={{ position: 'relative' }}>
      ...
      <Collapse in={!collapsed}>
        <Box ml="36" mt="8" mb="16">
          {address && (
            <Box mb="sm">
              <Text size="xs" fw={500} color="gray.6">Address</Text>
              {(address.line?.[0] || address.line?.[1] || address.city || address.state || address.postalCode) && (
                <MedplumLink to={`/Patient/${patientId}/edit`} style={{ textDecoration: 'none', color: 'black', display: 'block' }}>
                  <Group align="flex-start" gap={0} className={styles.patientSummaryListItem} style={{ position: 'relative', minWidth: 0 }}>
                    <Box style={{ flex: 1, minWidth: 0 }}>
                      {address.line?.[0] && (
                        <Text size="sm" fw={500} truncate style={{ minWidth: 0 }}>{address.line[0]}</Text>
                      )}
                      {address.line?.[1] && (
                        <Text size="sm" fw={500} truncate style={{ minWidth: 0 }}>{address.line[1]}</Text>
                      )}
