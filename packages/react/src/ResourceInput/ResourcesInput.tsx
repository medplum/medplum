...
export interface ResourcesInputProps<T extends Resource = Resource> {
...
export function ResourcesInput<T extends Resource = Resource>(props: ResourcesInputProps<T>): JSX.Element | null {
      ...
      Promise.allSettled(
        items.map((item): Promise<T> => {
          ...
          if (isReference(item) && isPopulated(item)) {
            return medplum.readReference(item as Reference<T>) as Promise<T>;
          }
