...
function buildSearchColumns(tableDefinition: TableDefinition, resourceType: string): void {
  const resourceTypeSearchParams = Object.values(getSearchParameters(resourceType) ?? {});
  const derivedSearchParams: SearchParameter[] = [];
  // console.log(`BEGIN ${resourceType} =====================`);
  const handled: string[] = [];
  for (const params of [resourceTypeSearchParams, derivedSearchParams]) {
  ...
  // console.log(`ALL HANDLED (${handled.length}):`, handled.join(', '));
