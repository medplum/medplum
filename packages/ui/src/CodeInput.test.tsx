...
function mockFetch(url: string, options: any): Promise<any> {
  ...
  if (url.endsWith('/fhir/R4/ValueSet/$expand?url=https%3A%2F%2Fexample.com%2Ftest&filter=xyz')) {
    result = valueSet;
  }
...
describe('CodeInput', () => {
  ...
  test('Renders Coding default value', () => {
    render(
      <MedplumProvider medplum={medplum} router={mockRouter}>
        <CodeInput property={statusProperty} name="test" defaultValue={{ code: 'abc' }} />
      </MedplumProvider>
    );
