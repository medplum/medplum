...
import { clearSchedulingParameter, getSchedulingParameters } from '@medplum/core';
...
export function ScheduleAvailabilityEditor(props: ScheduleAvailabilityEditorProps): JSX.Element {
  ...
  const [overriding, setOverriding] = useState(() =>
    schedule ? getSchedulingParameters(schedule, service, 'availability').length > 0 : true
  );
    ...
    try {
      if (props.schedule) {
        const updated = overriding
          ...
          : clearSchedulingParameter(props.schedule, service, 'availability');
