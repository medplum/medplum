 ...
 * already present cannot change the result set. Dropping it here keeps a degenerate query (the
 * same parameter repeated thousands of times) from reaching the SQL builder, the search links,
 * and the AuditEvent query, each of which scales with the filter count.
