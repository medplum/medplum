...
| `failed` | transient/ambiguous Bot-leg failure: 5xx, 429, response timeout, dispatch error, or row found in `processing` on startup (interrupted). Auto-retried only when the error code is transient (see §4.1) | `ChannelQueueWorker.markFailed` + `DurableQueue.recoverOnStartup` | yes (retry/review candidate) |
...
**Normal mode** (`autoRetry`, on by default): only the **transient** codes in `RETRYABLE_ERROR_CODES` retry — a retry can never cause duplicate delivery, because the server provably never accepted the message. The ambiguous codes stay in `failed` for operator review (never silently re-dispatched), and permanent codes land in `rejected`. This stays the case until the server supports callback-keyed dedupe.
...
| Ambiguous delivery (request sent, connection dropped before response) | **Normal mode:** not retried automatically — the row goes to `failed` for operator review (same stance as `recoverOnStartup`), because the server may already have processed it and re-dispatch would double-process. Only provably-unsent requests are requeued on disconnect. **Guaranteed-delivery mode (§4.1):** retried, duplication risk explicitly accepted. Future: callback-keyed dedupe in the server would make ambiguous auto-retry safe everywhere. |
