...
- **Bot/server leg** — `state` — the message's journey to the Medplum server and back,
  plus the worker scheduling lifecycle.
- **Source leg** — `ack_outcome` — whether the app-level ACK made it back to the
  sending device.

The cell that motivates the split: the Bot/server can accept a message (2xx) while the
source connection has already closed, so the ACK can't be returned. That is
`processed` + `undelivered` — **not** a Bot/server-leg failure, and never re-dispatched
(the Bot/server already has it). It recovers when the source retransmits, which replays
...
- **Checkpoint cadence**: WAL auto-checkpoints every 1000 pages (~4 MB). On top of that the `DurableQueue` owns a checkpoint loop — started in `open()`, stopped in `close()` — that runs a dirty-gated `PRAGMA wal_checkpoint(TRUNCATE)` every `DEFAULT_CHECKPOINT_INTERVAL_MS` (10 s) so the WAL drains shortly after traffic stops instead of lingering until the next big-page threshold. It runs **regardless of dispatch leadership** (a follower still accepts intake writes, so it has its own WAL to drain). The retention sweep also checkpoints at the end of each pass, and `close()` does a final flush.
...
| `channelArBehavior`                 | `valueString`  | `continue`                                                      | Agent-wide default for the per-channel `arBehavior` (below): what a channel does after a message lands terminally `rejected`. `continue` (the default, matching prior behavior) keeps draining past the reject; `pause` halts the channel until the reject is cleared. Overridden per channel by the `arBehavior` URL param.                 |
...
| `arBehavior`                 | `pause` \| `continue`              | agent setting (default `continue`)   | What the worker does after a message lands terminally `rejected` (an application reject — MSA-1 of `AR`/`CR`, or a permanent server 4xx). `pause` refuses to claim while the channel has a `rejected` row (enforced in `CLAIM_NEXT_PAUSE_ON_REJECT`, so it survives restarts; operator clears the reject to resume), preserving per-channel ordering when a later message may depend on the rejected one. `continue` keeps draining past the reject — for when the far side controls when `AR`/`CR` is sent and a single reject must not stall the pipe. Requires the durable queue; configuring it with the queue off logs a warning and has no effect. |

Auto-retry resolution is per-field: endpoint URL param → agent `channelRetryMode` / `channelAutoRetry*` setting → built-in default (see `resolveRetryPolicy` in `hl7.ts`). Invalid values warn and fall through to the next layer. `arBehavior` resolves the same way (`arBehavior` URL param → `channelArBehavior` setting → `continue`).
...
loop:
  ...
  try:
    ...
    // Bot/server-leg error, and is never re-dispatched (recovers via source retransmit).
...
### 9.1 Wiring Bot/server responses back to rows
...
drove them to the Bot/server, they would double-process upstream and fight over the
...
- Replay: a future `medplum-agent-replay <id>` CLI (out of scope for v1) would reset state to `queued`. For v1 we document the SQL: `UPDATE inbound_hl7_messages SET state='queued', attempt_count=0 WHERE id=?;` (only `failed` rows — never `rejected`, and never `processed`+`undelivered`, whose Bot/server leg already succeeded).
...
- INFO: enqueue (msg control id + row id), commit-ack sent, Bot/server response received, processed.
...
- ACK delivery failure: Bot/server returns 2xx but the source connection has closed → row is `processed`+`undelivered`, never re-dispatched (upstream hit exactly once); a retransmit replays the stored ACK and flips it to `delivered`.
