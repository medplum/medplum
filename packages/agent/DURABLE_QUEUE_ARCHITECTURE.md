...
- **v3** — logical channels: adds the `logical_channel_key` column (default `''`) and `idx_inbound_vchannel_claim` (§4.2).
- **v4** — adds `delayed` to the `uq_inbound_dup_active` predicate so a parked row still counts as an active duplicate (§4.2). Safe on a populated table: it is the first migration to introduce the `delayed` state, so no row is in it at apply time.
...
- Schema migration: open against an empty file (v0) → ends at the latest version (v4 today; adds `logical_channel_key` and `delayed`). Re-open is a no-op.
