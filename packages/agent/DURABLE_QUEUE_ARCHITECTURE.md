...
| `channelArBehavior`                 | `valueString`  | `pause`                                                         | Agent-wide default for the per-channel `arBehavior` (below): what a channel does after a message lands terminally `rejected`. `pause` halts the channel until the reject is cleared; `continue` keeps draining past it. Overridden per channel by the `arBehavior` URL param.                                                                |
...
| `arBehavior`                 | `pause` \| `continue`              | agent setting (default `pause`)      | What the worker does after a message lands terminally `rejected` (an application reject — MSA-1 of `AR`/`CR`, or a permanent server 4xx). `pause` refuses to claim while the channel has a `rejected` row (enforced in `CLAIM_NEXT_PAUSE_ON_REJECT`, so it survives restarts; operator clears the reject to resume), preserving per-channel ordering when a later message may depend on the rejected one. `continue` keeps draining past the reject — for when the far side controls when `AR`/`CR` is sent and a single reject must not stall the pipe. Requires the durable queue; configuring it with the queue off logs a warning and has no effect. |

Auto-retry resolution is per-field: endpoint URL param → agent `channelRetryMode` / `channelAutoRetry*` setting → built-in default (see `resolveRetryPolicy` in `hl7.ts`). Invalid values warn and fall through to the next layer. `arBehavior` resolves the same way (`arBehavior` URL param → `channelArBehavior` setting → `pause`).
