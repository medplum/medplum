...
/**
 * One HL7 field path in a channel's `virtualChannelKey` spec, parsed from a token
 ...
 */
export interface VirtualChannelField {
...
/**
 * Parses a `virtualChannelKey` spec into validated field paths.
 *
 ...
 */
export function parseVirtualChannelKeySpec(
  raw: string | undefined,
  logger: ILogger
): VirtualChannelField[] | undefined {
  const spec = (raw ?? '').trim();
  if (spec === '') {
    return [];
  }
  const fields: VirtualChannelField[] = [];
  for (const token of spec.split('-')) {
    const parsed = parseVirtualChannelField(token.trim());
    if (!parsed) {
      logger.warn(
        `Invalid virtualChannelKey field '${token}' in spec '${raw}'; ` +
 ...
 */
function parseVirtualChannelField(token: string): VirtualChannelField | undefined {
  if (!token) {
    return undefined;
  }
...
/**
 * Computes a message's virtual-channel key from a parsed spec.
 *
 ...
 * @param spec - The parsed field paths (from {@link parseVirtualChannelKeySpec}).
 * @returns The virtual-channel key for this message.
 */
export function computeVirtualChannelKey(message: Hl7Message, spec: VirtualChannelField[]): string {
  if (spec.length === 0) {
    return '';
  }
 ...
 */
function extractFieldValue(message: Hl7Message, field: VirtualChannelField): string {
