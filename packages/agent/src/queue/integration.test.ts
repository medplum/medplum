...
// keyed on virtualChannelKey=MSH.4 partitions messages by `facility`.
...
describe('Durable queue integration', () => {
  ...
  test('virtual channels dispatch concurrently across partitions while each partition stays serial', async () => {
    ...
    const [endpoint, port] = await createEndpointWithRandomPort(medplum, {
      ...BASE_ENDPOINT,
      address: 'mllp://0.0.0.0:0?enhanced=true&virtualChannelKey=MSH.4&maxWorkers=2',
    });
    const agent = await medplum.createResource<Agent>({
      resourceType: 'Agent',
      name: 'Durable Queue Test Agent',
    ...
    // Intake computed and stored the partition key end-to-end.
    expect(queue.findSeenByControlId('vc', 'A1')?.virtualChannelKey).toBe('MSH.4:HOSPA');
    expect(queue.findSeenByControlId('vc', 'B1')?.virtualChannelKey).toBe('MSH.4:HOSPB');

    // The pool dispatches A1 and B1 concurrently (distinct partitions), but NOT A2
    // (same partition as the still-in-flight A1). This is stable: A2 can't be
    // claimed until A1 settles, so the count can't creep past 2.
  ...
  test('auto-retry (guaranteed default): a transient 5xx is retried and then succeeds, reaching processed', async () => {
    ...
    startMockServer((cmd) => {
      dispatches++;
      // First dispatch fails transiently (5xx); every retry afterwards succeeds.
      if (dispatches === 1) {
        return {
          type: 'agent:transmit:response',
          channel: cmd.channel,
    ...
    const response = await client.sendAndWait(TEST_MSG('INT_RETRY'), {
      returnAck: ReturnAckCategory.FIRST,
      timeoutMs: 5000,
    });
    // Commit ACK minted at intake, independent of the (failing-then-recovering) Bot leg.
    expect(response.getSegment('MSA')?.getField(1)?.toString()).toBe('CA');

    // The transient failure was retried, not settled terminally: the row drains to
    // `processed` and the Bot was dispatched more than once (the failed attempt + a
    // succeeding retry).
    const queue = app.getDurableQueue() as DurableQueue;
    await waitForRow(queue, (counts) => counts.processed === 1, 5000);
    expect(dispatches).toBeGreaterThanOrEqual(2);
    const row = queue.findSeenByControlId('dq-test', 'INT_RETRY');
  ...
  test('auto-retry (guaranteed default): a 4xx without a definitive AR/CR ACK is retried, then succeeds', async () => {
    ...
    startMockServer((cmd) => {
      ...
      if (dispatches === 1) {
        return {
          ...
          contentType: ContentType.TEXT,
          statusCode: 422,
          body: 'transient validation hiccup', // not an HL7 ACK → no MSA-1, ambiguous
        } satisfies AgentTransmitResponse;
      }
      const ack = Hl7Message.parse(cmd.body).buildAck({ ackCode: 'AA' });
      return {
    ...
    const response = await client.sendAndWait(TEST_MSG('INT_4XX_RETRY'), {
      returnAck: ReturnAckCategory.FIRST,
      timeoutMs: 5000,
    });
    expect(response.getSegment('MSA')?.getField(1)?.toString()).toBe('CA');
    ...
    const queue = app.getDurableQueue() as DurableQueue;
    await waitForRow(queue, (counts) => counts.processed === 1, 5000);
    // Retried (not rejected) and ultimately delivered — the contrast with the
    // normal-mode reject test, where the very same 422 settles terminally.
    expect(dispatches).toBeGreaterThanOrEqual(2);
    expect(queue.findSeenByControlId('dq-test', 'INT_4XX_RETRY')?.state).toBe('processed');
    expect(queue.countByState()).toMatchObject({
      processed: 1,
      queued: 0,
      claimed: 0,
  ...
  // Note: periodic WAL checkpointing is now owned by the DurableQueue's own
  // checkpoint loop (open() starts it, close() stops it), not driven by the App
  // heartbeat. The loop is covered deterministically with fake timers in
  // durable-queue.test.ts ('open() runs a checkpoint loop ...'); the close()-time
  // flush is covered by the test below.
  ...
  test('loss of leadership mid-run: a demoted leader stops driving the queue and leaves its in-flight row for the new owner', async () => {
    ...
    // its CA: under enhanced mode that's the commit ACK minted at intake, independent
    // of the server.
    const transmits: string[] = [];
    startMockServer((cmd) => {
      transmits.push(Hl7Message.parse(cmd.body).getSegment('MSH')?.getField(10)?.toString() ?? '');
      return undefined; // withhold — wedge the leader's worker on the in-flight row
    });
    ...
    const agent = await medplum.createResource<Agent>({
      resourceType: 'Agent',
      name: 'Durable Queue Leader-Loss Agent',
      status: 'active',
      channel: [{ name: 'dq-test', endpoint: createReference(endpoint), targetReference: createReference(bot) }],
      setting: [
        { name: 'durableQueue', valueBoolean: true },
    ...
    const app = new App(medplum, agent.id, LogLevel.WARN);
    await app.start();
    await waitFor(() => app.isQueueLeader(), 5000, 'app acquires lease');

    // Source pushes three messages. LL_1 is claimed and dispatched (then wedges on the
    // withheld response); LL_2/LL_3 queue behind it (serial per channel). Each send
    // still resolves on the commit CA.
    const client = new Hl7Client({ host: 'localhost', port });
    for (const id of ['LL_1', 'LL_2', 'LL_3']) {
      const ack = await client.sendAndWait(TEST_MSG(id), { returnAck: ReturnAckCategory.FIRST, timeoutMs: 5000 });
      expect(ack.getSegment('MSA')?.getField(1)?.toString()).toBe('CA');
    }

    const queue = app.getDurableQueue() as DurableQueue;
    await waitFor(() => transmits.length === 1, 5000, 'leader dispatches LL_1');
    expect(transmits).toEqual(['LL_1']);
    expect(queue.countByState()).toMatchObject({ inflight: 1, queued: 2 });
