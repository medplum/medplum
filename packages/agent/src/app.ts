...
export class App {
  ...
  // Agent-wide virtual-channel defaults; an endpoint URL param overrides each when
  // a channel resolves its config. Undefined = not set (falls through to the default).
  private channelMaxWorkers: number | undefined;
  private channelVirtualChannelKey: string | undefined;
  ...
  private async beginReloadConfig(): Promise<{ listenersStarted: Promise<void> }> {
    ...
    // Agent-wide virtual-channel defaults; channels layer their endpoint URL
    // params (maxWorkers, virtualChannelKey) over these when resolving config.
    this.channelMaxWorkers = agent?.setting?.find((setting) => setting.name === 'channelMaxWorkers')?.valueInteger;
    this.channelVirtualChannelKey = agent?.setting?.find(
      (setting) => setting.name === 'channelVirtualChannelKey'
    )?.valueString;
  ...
  /** @returns The agent-wide `channelVirtualChannelKey` spec (per-channel partition default), or undefined when unset. */
  getChannelVirtualChannelKey(): string | undefined {
    return this.channelVirtualChannelKey;
  }
