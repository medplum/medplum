 ...
 *
 * ⚠️  IMPORTANT: This script assumes bots are already deployed to Medplum.
 *    For complete setup (build + deploy + subscribe), use: npm run setup
 *
 * Usage:
 ...
 * @param medplum - The Medplum client
 */
async function createAllSubscriptions(medplum) {
    ...
    for (const subscription of subscriptions) {
        try {
            ...
            const result = await createSubscription(medplum, subscription.botName, subscription.resourceType, subscription.criteria);
            results.push({
...
async function main() {
    try {
        ...
        console.log('3. Monitor bot execution in the Medplum console');
    }
    catch (error) {
