...
import { handler } from './b11-dsi-colonoscopy-bot';

describe('B11 DSI Colonoscopy Bot', () => {
  ...
  beforeEach(async () => {
    ...
    patient1 = await medplum.createResource({
      ...
      name: [{ given: ['Marge'], family: 'Strickland' }],
    });
  ...
  it('skips when patient has prior colonoscopy in last 5 years', async () => {
    ...
    // Create a prior colonoscopy procedure 2 years ago
    await medplum.createResource({
      ...
      subject: createReference(oldPatient),
      performedDateTime: new Date(today.getFullYear() - 2, 0, 1).toISOString(),
    });
  ...
  it('successfully creates a task when patient has colonoscopy older than 5 years', async () => {
    ...
    // Create a prior colonoscopy procedure 6 years ago
    await medplum.createResource({
      resourceType: 'Procedure',
      status: 'completed',
      code: {
      ...
      subject: createReference(oldPatient),
      performedDateTime: new Date(today.getFullYear() - 6, 0, 1).toISOString(),
    });
  ...
  it('creates a task with correct properties for eligible patient', async () => {
    ...
    expect(task.note?.[0]?.text).toContain('Colorectal Cancer Screening Recommendation');
    expect(task.note?.[0]?.text).toContain('Age 48 years (recommended: ≥45 years)');
    expect(task.note?.[0]?.text).toContain('no colonoscopy documented in the past 5 years');
  ...
  it('creates feedback questionnaire and links it to the task', async () => {
    ...
    const questionnaires = await medplum.searchResources('Questionnaire', [
      ['identifier', 'https://www.medplum.com/questionnaires|dsi-feedback-colonoscopy'],
    ]);
  ...
  it('handles patient exactly at minimum age threshold', async () => {
    ...
    const exactAgePatient = await medplum.createResource({
      ...patient1,
      birthDate: new Date(today.getFullYear() - 45, 0, 1).toISOString().split('T')[0],
    });
