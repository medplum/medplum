...
export function TaskFilterMenu(props: TaskFilterMenuProps): JSX.Element {
  ...
  const uniquePerformerTypes = performerTypes?.filter((performerType, index, self) => {
    const identifier = performerType.coding?.[0]?.code || performerType.text;
    return identifier && self.findIndex(pt => 
      (pt.coding?.[0]?.code || pt.text) === identifier
    ) === index;
  }) || [];
              ...
              {uniquePerformerTypes.length > 0 ? (
                uniquePerformerTypes.map((type, index) => (
                  <Menu.Item 
                    ...
                    rightSection={performerType?.coding?.[0]?.code === type.coding?.[0]?.code ? <IconCheck size={16} /> : null}
                  >
                    <Text size="sm">
                      {type.coding?.[0]?.display ?? type.coding?.[0]?.code ?? 'Unknown'}
                    </Text>
                  </Menu.Item>
                ))
              ) : (
                <Menu.Item disabled>
                  <Text size="sm" c="dimmed">No performer types available</Text>
                </Menu.Item>
              )}
            </Menu.Dropdown>
          </Menu>
