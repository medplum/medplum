...
import { Paper, Stack, Text, Group, Badge, Divider, Loader, Button, Timeline, ThemeIcon } from '@mantine/core';
import { formatDate, formatHumanName } from '@medplum/core';
import type { ServiceRequest, HumanName, DocumentReference, DiagnosticReport, QuestionnaireResponse } from '@medplum/fhirtypes';
...
export function LabOrderDetails({ order, onOrderChange: _onOrderChange, diagnosticReports: allDiagnosticReports, activeTab }: LabOrderDetailsProps): JSX.Element {
    ...
    const steps: ProgressStep[] = [
      {
        ...
        status: 'completed',
        timestamp: order.authoredOn || order.meta?.lastUpdated ? formatDate(order.authoredOn || order.meta?.lastUpdated) : undefined
      },
      {
        id: 'lab-acknowledged',
      ...
      if (order.supportingInfo && order.supportingInfo.length > 0) {
        ...
        if (questionnaireRef?.reference) {
          try {
            ...
            const response = await medplum.readResource('QuestionnaireResponse', questionnaireRef.reference.split('/')[1]);
          ...
          if (searchResult && searchResult.length > 0) {
            ...
            const originalOrder = searchResult.find((sr: any) => {
              if (sr.basedOn && sr.basedOn.length > 0) {
                const hasBasedOnMatch = sr.basedOn.some((ref: any) => 
            ...
            if (originalOrder) {
              ...
              const serviceRequestWithQuestionnaire = searchResult.find((sr: any) => 
                sr.supportingInfo?.some((ref: any) => 
                  ref.reference?.startsWith('QuestionnaireResponse/')
                )
              );
  ...
  return (
    <Paper h="100%" style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <Stack gap="md" p="md">
          <Stack gap="md">
            ...
            <Group justify="space-between" align="center">
              <Group gap="xs">
                <Button
                  className={cx(classes.button, { [classes.selected]: activeDetailTab === (activeTab === 'open' ? 'progress' : 'report') })}
                  h={32}
                  radius="xl"
      ...
      <div style={{ 
        ...
        {activeDetailTab === 'order' && (
          <Stack gap="md">
            ...
            <Group align="flex-start" gap="lg">
              <Text fw={500} size="sm" style={{ width: '150px' }} c="dimmed">Order Date</Text>
              <Text size="sm">{formatDate(order.authoredOn || order.meta?.lastUpdated)}</Text>
            </Group>

            {order.code?.coding && (
              <Group align="flex-start" gap="lg">
                <Text fw={500} size="sm" style={{ width: '150px' }} c="dimmed">Test Code</Text>
                <Stack gap="xs">
            ...
            {requester?.resourceType === 'Practitioner' && (
              <Group align="flex-start" gap="lg">
                <Text fw={500} size="sm" style={{ width: '150px' }} c="dimmed">Ordering provider</Text>
            ...
            {order.performer?.[0]?.display && (
              <Group align="flex-start" gap="lg">
                <Text fw={500} size="sm" style={{ width: '150px' }} c="dimmed">Performing lab</Text>
            ...
            {order.requisition?.value && (
              <Group align="flex-start" gap="lg">
                <Text fw={500} size="sm" style={{ width: '150px' }} c="dimmed">Requisition ID</Text>
            ...
            {patient?.resourceType === 'Patient' && (
              <Group align="flex-start" gap="lg">
                <Text fw={500} size="sm" style={{ width: '150px' }} c="dimmed">Patient</Text>
            ...
            {order.priority && (
              <Group align="flex-start" gap="lg">
                <Text fw={500} size="sm" style={{ width: '150px' }} c="dimmed">Priority</Text>
            ...
            {order.reasonCode && (
              <Group align="flex-start" gap="lg">
                <Text fw={500} size="sm" style={{ width: '150px' }} c="dimmed">Reason</Text>
                <Stack gap="xs">
            ...
            {order.note && (
              <Group align="flex-start" gap="lg">
                <Text fw={500} size="sm" style={{ width: '150px' }} c="dimmed">Notes</Text>
                <Stack gap="xs">
            ...
            {order.orderDetail && (
              <>
                <Divider />
                <Stack gap="sm">
                  <Text fw={800} size="lg">Order Details</Text>
                  {order.orderDetail.map((detail, index) => (
                    <Group key={index} align="flex-start">
                      <Text fw={500} size="sm">Detail {index + 1}:</Text>
                      <Text size="sm">
                        {detail.text || detail.coding?.[0]?.display}
                      </Text>
                    </Group>
                  ))}
                </Stack>
            ...
            <Divider />
            <Stack gap="lg" mb="xl">
                <Text fw={800} size="md" pb="0">Requisition Document</Text>                
              {loadingDocs && (
                <Group>
                  <Loader size="sm" />
                  <Text size="sm" c="dimmed">Loading requisition documents...</Text>
                </Group>
              )}
                      ...
                      {doc.content && doc.content.length > 0 && (
                        <Stack gap="xs">
                          {doc.content.map((content, contentIndex) => (
                            <div key={contentIndex} style={{ height: '600px', borderRadius: '4px', overflow: 'hidden', border: '1px solid #3C3C3C' }}>
                              <style>
                                {`
              ...
              {!loadingDocs && labOrderRequisitionDocs.length === 0 && (
                <Stack gap="xs">
                  <Text size="sm" c="dimmed">No lab order requisition documents found.</Text>
                </Stack>
              )}
            </Stack>
            ...
            {questionnaireResponse && (
              <>
                <Divider />
                <Stack gap="md" mb="xl">
                  <Text fw={800} size="md">Order Entry Questions</Text>
                  
                  {loadingQuestionnaire && (
                    <Group>
                      <Loader size="sm" />
                      <Text size="sm" c="dimmed">Loading questionnaire response...</Text>
                    </Group>
                  )}
                            ...
                            <Group key={index} align="flex-start" style={{ alignItems: 'flex-start' }} gap="lg">
                              ...
                              <div style={{ flex: 1 }}>
                                {item.answer && item.answer.length > 0 ? (
                                  ...
                                  <Text size="sm" c="dimmed">No answer provided</Text>
                                )}
                              </div>
                            </Group>
                          ))
                      ) : (
                        <Text size="sm" c="dimmed">No questionnaire items found.</Text>
                      )}
                    </Stack>
                  )}
                </Stack>
            ...
            <Divider />
            <Stack gap="lg" mb="xl">
                <Text fw={800} size="md" pb="0">Specimen Label</Text>                
              {loadingSpecimenDocs && (
                <Group>
                  <Loader size="sm" />
                  <Text size="sm" c="dimmed">Loading specimen label documents...</Text>
                </Group>
              )}
              ...
              {!loadingSpecimenDocs && specimenLabelDocs.length === 0 && (
                <Text size="sm" c="dimmed">No specimen label documents found.</Text>
              )}
            </Stack>
        ...
        {activeDetailTab === 'report' && activeTab === 'completed' && primaryReport && (
          ...
          {primaryReport.result && primaryReport.result.length > 0 && (
            ...
            <Text fw={500} size="sm" style={{ minWidth: '150px' }} c="dimmed">Report Status</Text>
            <Text size="sm" style={{ textTransform: 'capitalize' }}>{primaryReport.status}</Text>
            </Group>

            {primaryReport.issued && (
              <Group align="flex-start">
                <Text fw={500} size="sm" style={{ minWidth: '150px' }} c="dimmed">Issue Date</Text>
            ...
            {primaryReport.conclusion && (
              <Group align="flex-start">
                <Text fw={500} size="sm" style={{ minWidth: '150px' }} c="dimmed">Interpretation</Text>
            ...
            {primaryReport?.presentedForm && primaryReport.presentedForm.length > 0 && (
              <>
                <Divider mt="xl"/>
                <Stack gap="lg" mb="xl">
                  <Text fw={800} size="md" pb="0">Lab Document</Text>
                  <Stack gap="md">
                    {primaryReport.presentedForm.map((form, index) => (
                      <Stack key={index} gap="xs">
                        <div style={{ height: '600px', borderRadius: '4px', overflow: 'hidden', border: '1px solid #3C3C3C' }}>
                          <style>
                            {`
                              div[data-testid="attachment-iframe"] {
                                height: 600px !important;
