...
function expiredButAvailableResponse(): Parameters {
  return {
    ...
    parameter: [
      ...
      { name: 'warning', valueString: 'SMART Health Link is expired. Records were still available and decrypted.' },
      { name: 'fhirResources', valueString: JSON.stringify([SHARED_BUNDLE]) },
    ],
  };
}

const EXPIRED_SHLINK =
  'shlink:/eyJ1cmwiOiJodHRwczovL2lzc3Vlci5leGFtcGxlLmNvbS9wYXlsb2FkIiwiZmxhZyI6IlUiLCJrZXkiOiJBQUVDQXdRRkJnY0lDUW9MREEwT0R4QVJFaE1VRlJZWEdCa2FHeHdkSGg4IiwiZXhwIjoxMDAwMDAwMDAwLCJ2IjoxfQ==';
...
describe('SmartHealthLinkImport expired links', () => {
    ...
    return render(
      <MemoryRouter>
        <MedplumProvider medplum={medplum}>
          <MantineProvider>
            <Notifications />
            <SmartHealthLinkImport variant="modal" />
          </MantineProvider>
        </MedplumProvider>
      </MemoryRouter>
    );
  ...
  test('Does not repeat the expiry warning in the generic warning banner', async () => {
    ...
    expect(screen.queryByText(/Records were still available and decrypted/i)).not.toBeInTheDocument();
  });
  ...
  test('Surfaces the server error when an expired link is no longer available', async () => {
    vi.spyOn(medplum, 'post').mockResolvedValue({
      ...
      parameter: [
        ...
        { name: 'error', valueString: 'SMART Health Link has expired and its records are no longer available' },
      ],
    });
    await resolveExpiredLink();

    await waitFor(() => {
      expect(screen.getByText(/expired and its records are no longer available/i)).toBeInTheDocument();
    });
