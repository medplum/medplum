 ...
 *   Organization, or undefined when the save itself failed. The hook has already shown the error
 *   notification by then, so callers only need to keep their form open. A failed Candid
 *   registration leaves the saved Organization in place, unregistered, so saving again retries it.
 */
...
export function useBillingOrganizations(): BillingOrganizations {
  ...
  const registerWithCandid = useCallback(
    async (organization: WithId<Organization>): Promise<void> => {
      try {
  ...
  const saveOrganization = useCallback(
    ...
    [medplum, candidBotId, registerWithCandid]
  );
